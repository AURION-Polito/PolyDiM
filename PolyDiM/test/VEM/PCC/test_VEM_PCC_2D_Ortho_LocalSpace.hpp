...
Test_VEM_PCC_2D_Ortho_Polygon_Geometry Test_VEM_PCC_2D_Ortho_Geometry(
    const Gedim::GeometryUtilities &geometry_utilities)
{
    ...
    polygonVertices.row(0) << 2.8000000000000004e-03, 3.7200000000000004e-02, 3.6900000000000002e-02,
        3.3300000000000003e-02, 2.5600000000000001e-02, 1.4400000000000000e-02, 6.7000000000000002e-03,
        3.0999999999999999e-03;
    polygonVertices.row(1) << 2.1499999999999998e-02, 2.1499999999999998e-02, 2.9500000000000002e-02,
        4.0599999999999997e-02, 5.0700000000000002e-02, 5.0700000000000002e-02, 4.0599999999999997e-02,
    ...
    result.EdgesLength << 3.4400000000000000e-02, 8.0056230238501787e-03, 1.1669190203266030e-02,
        1.2700393694685222e-02, 1.1200000000000002e-02, 1.2700393694685220e-02, 1.1669190203266030e-02,
    ...
    edgeTangents.row(0) << 3.4400000000000000e-02, -3.0000000000000165e-04, -3.5999999999999990e-03,
        -7.7000000000000020e-03, -1.1200000000000002e-02, -7.6999999999999994e-03, -3.6000000000000003e-03,
        -2.9999999999999949e-04;
    edgeTangents.row(1) << 0.0000000000000000e+00, 8.0000000000000036e-03, 1.1099999999999995e-02,
        1.0100000000000005e-02, 0.0000000000000000e+00, -1.0100000000000005e-02, -1.1099999999999995e-02,
        -8.0000000000000036e-03;

    Eigen::MatrixXd &edgeNormals = result.EdgesNormal;
    edgeNormals.setZero(3, 8);
    edgeNormals.row(0) << 0.0000000000000000e+00, 9.9929761570918052e-01, 9.5122281894876248e-01,
        7.9525093810490199e-01, 0.0000000000000000e+00, -7.9525093810490211e-01, -9.5122281894876248e-01,
        -9.9929761570918074e-01;
    edgeNormals.row(1) << -1.0000000000000000e+00, 3.7473660589094460e-02, 3.0850469803743652e-01,
        6.0628041815918254e-01, 1.0000000000000000e+00, 6.0628041815918243e-01, 3.0850469803743663e-01,
        3.7473660589094196e-02;

    // Map triangle points
    std::vector<unsigned int> polygonTriangulation =
        geometry_utilities.PolygonTriangulationByFirstVertex(polygonVertices);
...
TEST(Test_VEM_PCC, Test_VEM_PCC_2D_Ortho_O1)
{
    ...
    ASSERT_TRUE(
        geometry_utilities.IsValueGreaterOrEqual(7.0e-15, result.ErrorPiNabla, geometry_utilities.Tolerance1D()));
    ASSERT_TRUE(
        geometry_utilities.IsValueGreaterOrEqual(1.6e-14, result.ErrorPi0km1, geometry_utilities.Tolerance1D()));
    ...
    for (unsigned int d = 0; d < 2; ++d)
        ASSERT_TRUE(geometry_utilities.IsValueGreaterOrEqual(
            2.6e-14, result.ErrorPi0km1Grad[d], geometry_utilities.Tolerance1D()));
    ASSERT_TRUE(
        geometry_utilities.IsValueGreaterOrEqual(3.5e-14, result.ErrorStabilization, geometry_utilities.Tolerance1D()));
}
...
TEST(Test_VEM_PCC, Test_VEM_PCC_2D_Ortho_O2)
{
    ...
    const double relErrPiNabla = (local_space.PiNabla - refPiNabla).norm() / refPiNabla.norm();
    ASSERT_TRUE(
        geometry_utilities.IsValueGreaterOrEqual(1.5e-14, relErrPiNabla, std::numeric_limits<double>::epsilon()));
    // Test VEM performances
    Polydim::VEM::PCC::VEM_PCC_PerformanceAnalysis performanceAnalysis;
    ...
    for (unsigned int d = 0; d < 2; ++d)
        ASSERT_TRUE(geometry_utilities.IsValueGreaterOrEqual(
            2.6e-14, result.ErrorPi0km1Grad[d], geometry_utilities.Tolerance1D()));
    ASSERT_TRUE(
        geometry_utilities.IsValueGreaterOrEqual(3.9e-13, result.ErrorStabilization, geometry_utilities.Tolerance1D()));
}
...
TEST(Test_VEM_PCC, Test_VEM_PCC_2D_Ortho_O3)
{
    ...
    ASSERT_TRUE(
        geometry_utilities.IsValueGreaterOrEqual(7.0e-15, result.ErrorPiNabla, geometry_utilities.Tolerance1D()));
    ASSERT_TRUE(
        geometry_utilities.IsValueGreaterOrEqual(1.7e-14, result.ErrorPi0km1, geometry_utilities.Tolerance1D()));
    ...
    for (unsigned int d = 0; d < 2; ++d)
        ASSERT_TRUE(geometry_utilities.IsValueGreaterOrEqual(
            2.6e-14, result.ErrorPi0km1Grad[d], geometry_utilities.Tolerance1D()));
    ASSERT_TRUE(
        geometry_utilities.IsValueGreaterOrEqual(2.6e-12, result.ErrorStabilization, geometry_utilities.Tolerance1D()));
}
} // namespace UnitTesting
} // namespace Polydim
