...
{
// ***************************************************************************
std::vector<Eigen::VectorXd> MapInvVelocityValuesVect(const Polydim::FEM::MCC::FEM_Triangle_RT_MCC_2D_LocalSpace_Data &local_space,
        ...
        for (unsigned int o = 0; o < 5; o++)
        {
            ...
            for (unsigned int e = 0; e < 3; e++)
            {
                ...
                const std::vector<Eigen::MatrixXd> velocityBasisValues_edge = reference_element.EvaluateVelocityBasisFunctions(
                    reference_element_data.BoundaryQuadrature.Quadrature.Points.middleCols(
                ...
                Eigen::MatrixXd quantity = (velocityBasisValues_edge[0] * reference_element_data.EdgeNormals(0, e) +
                                            velocityBasisValues_edge[1] * reference_element_data.EdgeNormals(1, e))
                                               .transpose() *
                                           reference_element_data.BoundaryQuadrature.Quadrature.Weights
...
//         Eigen::VectorXd pressure_boundary = vander_boundary.col(1) + vander_boundary.col(2) + 0.5 *
//         vander_boundary.col(0); std::vector<Eigen::VectorXd> velocity_boundary = {-(vander_boundary_der[0].col(1) +
//         vander_boundary_der[0].col(2)),
//                                                           -(vander_boundary_der[1].col(1) +
//                                                           vander_boundary_der[1].col(2))};

//         Eigen::VectorXd pressure_internal = vander_internal.col(1) + vander_internal.col(2) + 0.5 *
//         vander_internal.col(0); std::vector<Eigen::VectorXd> velocity_internal = {-(vander_internal_der[0].col(1) +
//         vander_internal_der[0].col(2)),
//                                                           -(vander_internal_der[1].col(1) +
//                                                           vander_internal_der[1].col(2))};
...
//                         .segment(e * (reference_element_data.Order + 1), (reference_element_data.Order + 1))
//                         .asDiagonal() *
//                     velocity_boundary[0].middleRows(e * (reference_element_data.Order + 1),
//                     (reference_element_data.Order + 1)) +
//                 reference_element_data.VanderBoundary1D.transpose() *
//                     reference_element_data.BoundaryQuadrature.WeightsTimesNormal[1]
//                         .segment(e * (reference_element_data.Order + 1), (reference_element_data.Order + 1))
//                         .asDiagonal() *
//                     velocity_boundary[1].middleRows(e * (reference_element_data.Order + 1),
//                     (reference_element_data.Order + 1));
...
//             Dmatrix.block(3 * (reference_element_data.Order + 1) + reference_element_data.Nkm1, 0,
//             reference_element_data.Nkm1, 1) =
//                 reference_element_data.reference_element_data_pressure.ReferenceBasisFunctionValues
//                     .leftCols(reference_element_data.Nkm1)
//                     .transpose() *
//                 reference_element_data.Quadrature.ReferenceTriangleQuadrature.Weights.asDiagonal() *
...
//         Eigen::MatrixXd monomials_values =
//             monomials_2D.Vander(reference_element_data.monomials_2D_data, referenceQuadrature.Points,
...
//                                             + velocityBasisValues[1].transpose() * weights.asDiagonal() *
//                                             velocity[1];
...
//         Dmatrix_pressure << 0.5 + reference_element_data.monomials_2D_center(0) +
//         reference_element_data.monomials_2D_center(1), 1.0, 1.0;

//         Eigen::VectorXd discrete_pressure = pressureBasisValues * Dmatrix_pressure;

//         // const Eigen::VectorXd velocity_error = global_A.block(6, 6, 2, 2) * Dmatrix.bottomRows(2) -
//         local_rhs_A.bottomRows(2)  - (- global_A.block(6, 0, 2, 6) * Dmatrix.topRows(6)); const Eigen::VectorXd
//         value_error = diverVelocityBasisFunctions.rightCols(2).transpose() * weights.asDiagonal()  *
//         pressureBasisValues * Dmatrix_pressure; const Eigen::VectorXd velocity_error = global_A.block(6, 6, 2, 2) *
//         Dmatrix.bottomRows(2) - diverVelocityBasisFunctions.rightCols(2).transpose() * weights.asDiagonal()  *
//         pressureBasisValues * Dmatrix_pressure
//                                                - (- global_A.block(6, 0, 2, 6) * Dmatrix.topRows(6));
//         const Eigen::VectorXd div_velocity_error = pressureBasisValues.transpose() * weights.asDiagonal() *
//         diverVelocityBasisFunctions.rightCols(2) * Dmatrix.bottomRows(2)
//                                                    + pressureBasisValues.transpose() * weights.asDiagonal()  *
//                                                    pressureBasisValues * Dmatrix_pressure
//                                                    - (pressureBasisValues.transpose() * weights.asDiagonal()  *
//                                                    source_term
//                                                       - pressureBasisValues.transpose() * weights.asDiagonal() *
//                                                       diverVelocityBasisFunctions.leftCols(6) * Dmatrix.topRows(6));
...
//         neumann_matrix.block(2, 0, 3, 6) = pressureBasisValues.transpose() * weights.asDiagonal() *
//         diverVelocityBasisFunctions.leftCols(6);

//         global_matrix.block(2, 0, 3, 2) = pressureBasisValues.transpose() * weights.asDiagonal() *
//         diverVelocityBasisFunctions.rightCols(2); global_matrix.block(2, 2, 3, 3) = pressureBasisValues.transpose() *
//         weights.asDiagonal()  * pressureBasisValues;

//         const double max_error_vel = velocity_error.cwiseAbs().maxCoeff();

//         const double max_error = (global_matrix * solution - (global_rhs - neumann_matrix *
//         neuman_solution)).cwiseAbs().maxCoeff(); const double max_error_div =
//         div_velocity_error.cwiseAbs().maxCoeff();
    ...
    for (unsigned int p = 0; p < edge_directions.size(); p++)
    {
        for (unsigned int o = 1; o < 2; o++)
        {
            ...
            for (unsigned int e = 0; e < 3; e++)
            {
                ...
                if (p == 0 && e == 1)
                    std::cout << (velocityBasisValues_edge[0] * polygon_edges_normal(0, e) +
...
TEST(Test_FEM_Triangle_RT_MCC_2D, Test_FEM_Triangle_RT_MCC_2D_Local_Space_Patch)
{
    ...
    const auto mesh_geometric_data =
        Polydim::PDETools::Mesh::PDE_Mesh_Utilities::compute_mesh_2D_geometry_data(geometry_utilities, meshUtilities, mesh);
    ...
    for (unsigned int c = 0; c < mesh.Cell2DTotalNumber(); c++)
    {
        ...
        const auto monomials_values_der = monomials_2D.VanderDerivatives(
            reference_element_data.FEM_ReferenceElement_Data.rt_triangle_reference_element_data.monomials_2D_data,
            monomials_values,
            1.0);
        ...
        Eigen::VectorXd source_term = pressure;
        Eigen::VectorXd Dmatrix_pressure = (pressureBasisValues.transpose() * weights.asDiagonal() * pressureBasisValues)
                                               .llt()
                                               .solve(pressureBasisValues.transpose() * weights.asDiagonal() * source_term);
        ...
        const auto vander_internal_der = monomials_2D.VanderDerivatives(
            reference_element_data.FEM_ReferenceElement_Data.rt_triangle_reference_element_data.monomials_2D_data,
            vander_internal,
            1.0);
        ...
        Eigen::VectorXd Dmatrix =
            Eigen::MatrixXd::Zero(reference_element_data.FEM_ReferenceElement_Data.rt_triangle_reference_element_data
                                      .reference_element_data_velocity.NumBasisFunctions,
                                  1);
        ...
        for (unsigned int e = 0; e < 3; e++)
        {
            ...
            const auto vander_boundary_der = monomials_2D.VanderDerivatives(
                reference_element_data.FEM_ReferenceElement_Data.rt_triangle_reference_element_data.monomials_2D_data,
                vander_boundary,
                1.0);
            ...
            Dmatrix.block(e * (reference_element_data.Order + 1), 0, (reference_element_data.Order + 1), 1) =
                direction *
                    reference_element_data.FEM_ReferenceElement_Data.rt_triangle_reference_element_data.VanderBoundary1D.transpose() *
                    (polygon_edges_normal(0, e) * boundary_quadrature[e].Weights).asDiagonal() * velocity_boundary[0] +
                direction *
                    reference_element_data.FEM_ReferenceElement_Data.rt_triangle_reference_element_data.VanderBoundary1D.transpose() *
                    (polygon_edges_normal(1, e) * boundary_quadrature[e].Weights).asDiagonal() * velocity_boundary[1];
        }
        ...
        if (reference_element_data.Order > 0)
        {
            Dmatrix.block(3 * (reference_element_data.Order + 1),
                          0,
                          reference_element_data.FEM_ReferenceElement_Data.rt_triangle_reference_element_data.Nkm1,
                          1) =
                reference_element_data.FEM_ReferenceElement_Data.rt_triangle_reference_element_data
                    .reference_element_data_pressure.ReferenceBasisFunctionValues
                    .leftCols(reference_element_data.FEM_ReferenceElement_Data.rt_triangle_reference_element_data.Nkm1)
                    .transpose() *
                reference_element_data.FEM_ReferenceElement_Data.rt_triangle_reference_element_data.Quadrature
                    .ReferenceTriangleQuadrature.Weights.asDiagonal() *
                MapInvVelocityValuesVect(local_space_data.FEM_LocalSpace_Data.rt_triangle_local_space_data, velocity_internal)[0];
            ...
            Dmatrix.block(3 * (reference_element_data.Order + 1) +
                ...
                reference_element_data.FEM_ReferenceElement_Data.rt_triangle_reference_element_data.Quadrature
                    .ReferenceTriangleQuadrature.Weights.asDiagonal() *
                MapInvVelocityValuesVect(local_space_data.FEM_LocalSpace_Data.rt_triangle_local_space_data, velocity_internal)[1];
        }
    ...
    for (unsigned int c = 0; c < mesh.Cell2DTotalNumber(); c++)
    {
        ...
        for (unsigned int e = 0; e < 3; e++)
        {
            ...
            const std::vector<Eigen::MatrixXd> velocityBasisValues_edge =
                Polydim::PDETools::LocalSpace_MCC_2D::VelocityBasisFunctionsValues(
                    reference_element_data,
                    local_space_data,
            ...
            ASSERT_TRUE((((velocityBasisValues_edge[0] * mesh_geometric_data.Cell2DsEdgeNormals.at(c)(0, e) +
                           velocityBasisValues_edge[1] * mesh_geometric_data.Cell2DsEdgeNormals.at(c)(1, e))
                              .transpose() *
                          local_space_data.FEM_LocalSpace_Data.BoundaryQuadrature[e].Weights.asDiagonal() *
                          reference_element_data.FEM_ReferenceElement_Data.rt_triangle_reference_element_data.VanderBoundary1D)
                             .sum() -
                         direction * (o + 1)) < 1.0e-12);
        ...
        local_B[c] = pressure_basis_functions_values.transpose() * cell2D_internal_quadrature.Weights.asDiagonal() *
...

    {
        ...
        const Eigen::VectorXd Dmatrix_0 = dofs_values_0.segment(0, local_count_dofs_0.num_total_dofs - num_local_dofs_pressure);
        ...
        const Eigen::VectorXd Dmatrix_2 = dofs_values_2.segment(0, local_count_dofs_2.num_total_dofs - num_local_dofs_pressure);
...

    {
        ...
        const Eigen::VectorXd Dmatrix_1 = dofs_values_1.segment(0, local_count_dofs_1.num_total_dofs - num_local_dofs_pressure);
        ...
        const double max_error_1 = (global_matrix_1 * local_solution_1 - (global_rhs_1 - neumann_matrix_1 * local_neuman_solution_1))
                                       .cwiseAbs()
                                       .maxCoeff();
...

    {
        ...
        const Eigen::VectorXd Dmatrix_3 = dofs_values_3.segment(0, local_count_dofs_3.num_total_dofs - num_local_dofs_pressure);
        ...
        const double max_error_3 = (global_matrix_3 * local_solution_3 - (global_rhs_3 - neumann_matrix_3 * local_neuman_solution_3))
                                       .cwiseAbs()
                                       .maxCoeff();
