...
namespace Polydim
{
  namespace UnitTesting
  {
    TEST(Test_FEM_Triangle_PCC_2D, Test_FEM_Triangle_PCC_2D_Reference_Element)
    {
      ...
      std::array<Eigen::Vector3d, 3> boundaries_normal;
      boundaries_normal[0] = Eigen::Vector3d(0.0, -1.0, 0.0).normalized();
      boundaries_normal[1] = Eigen::Vector3d(+1.0, +1.0, 0.0).normalized();
      boundaries_normal[1] = Eigen::Vector3d(-1.0, 0.0, 0.0).normalized();
      std::array<Gedim::Quadrature::QuadratureData, 3> boundaries_quadrature;
      ...
      for (unsigned int o = 0; o < 5; o++)
      {
        ...
        const auto& internal_quadrature = reference_element_data.ReferenceTriangleQuadrature;
    ...
    TEST(Test_FEM_Triangle_PCC_2D, Test_FEM_Triangle_PCC_2D_Local_Space)
    {
      ...
      {
        const auto reference_element_data = reference_element.Create(o);
        const auto local_space_data = local_space.CreateLocalSpace(reference_element_data,
                                                                   polygon_geometry);
        ...
        const auto basisValues = local_space.ComputeBasisFunctionsValues(reference_element_data,
                                                                                    local_space_data,
                                                                                    points);
