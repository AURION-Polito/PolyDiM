...
namespace Polydim
{
  namespace UnitTesting
  {
    ...
    TEST(TEST_assembler_non_linear, TEST_assembler_non_linear_example)
    {
      ...
      const auto trial_mesh_dofs_info = Polydim::PDETools::LocalSpace_PCC_2D::SetMeshDOFsInfo(trial_reference_element_data, mesh, boundary_info);
      const auto trial_dofs_data = dofManager.CreateDOFs_2D(trial_mesh_dofs_info, mesh_connectivity_data);

      const auto test_mesh_dofs_info = Polydim::PDETools::LocalSpace_PCC_2D::SetMeshDOFsInfo(test_reference_element_data, mesh, boundary_info);
      ...
      auto advection_term_function = [](const double &x, const double &y, const double &z, const double& u_k, const std::array<double, 3>& u_grad_k) {
        return std::array<double, 3>({ u_k, u_k, 0.0 });
      };

      auto reaction_term_function = [](const double &x, const double &y, const double &z, const double& u_k, const std::array<double, 3>& u_grad_k) {
