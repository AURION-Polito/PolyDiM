...
TEST(TEST_assembler_stokes_PCC_2D, TEST_assembler_stokes_PCC_2D_example)
{
    ...
    auto pressure_gradient_solution_function = [](const double &x, const double &y, const double &z) {
        return std::array<double, 3>(
            {+2.0 * M_PI * cos(2.0 * M_PI * x) * cos(2.0 * M_PI * y), -2.0 * M_PI * sin(2.0 * M_PI * x) * sin(2.0 * M_PI * y), 0.0});
    };
...

    {
...

        {
            ...
            const auto J =
                PDETools::Assembler_Utilities::PCC_2D::to_Eigen_SparseArray(J_A_x + J_A_y - J_B_x - J_B_y - J_BT_x - J_BT_y);
