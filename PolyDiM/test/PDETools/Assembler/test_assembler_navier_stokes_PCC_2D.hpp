...
TEST(TEST_assembler_navier_stokes_PCC_2D, TEST_assembler_navier_stokes_PCC_2D_example)
{
    ...
    const std::string exportFolder = "./Export/TEST_assembler_stokes_PCC_2D/"
                                     "TEST_assembler_navier_stokes_PCC_2D_example";
    ...
    auto pressure_exact_solution_function = [](const double &x, const double &y, const double &z) {
        return std::numbers::pi * std::numbers::pi * sin(2.0 * std::numbers::pi * x) * cos(2.0 * std::numbers::pi * y);
    };
    ...
    auto pressure_gradient_solution_function = [](const double &x, const double &y, const double &z) {
        return std::array<double, 3>({2.0 * std::numbers::pi * std::numbers::pi * std::numbers::pi *
                                          cos(2.0 * std::numbers::pi * x) * cos(2.0 * std::numbers::pi * y),
    ...
    Gedim::Eigen_SparseArray<> J_Stokes;
    {
        ...
        const auto J_BT_x = PDETools::Assembler_Utilities::PCC_2D::to_SparseMatrix(
            PDETools::Assembler_Utilities::PCC_2D::to_Eigen_SparseArray(B_x.operator_dofs,
                                                                        ...
                                                                        {2 * velocity_dofs_data.NumberDOFs, 0},
                                                                        true));
        const auto J_BT_y = PDETools::Assembler_Utilities::PCC_2D::to_SparseMatrix(PDETools::Assembler_Utilities::PCC_2D::to_Eigen_SparseArray(
            B_y.operator_dofs,
            {tot_dofs, tot_dofs},
            {2 * velocity_dofs_data.NumberDOFs, velocity_dofs_data.NumberDOFs},
            true));
    ...
    while (num_iteration < max_iterations && residual_norm > newton_tol * solution_norm)
    {
...

        {
            ...
            Eigen::VectorXd du;
            {
                const auto J = PDETools::Assembler_Utilities::PCC_2D::to_Eigen_SparseArray(J_S + J_C);
        ...
        solution_norm = std::sqrt(u_x_error_L2.numeric_norm_L2 * u_x_error_L2.numeric_norm_L2 +
                                  u_y_error_L2.numeric_norm_L2 * u_y_error_L2.numeric_norm_L2 +
                                  p_error_L2.numeric_norm_L2 * p_error_L2.numeric_norm_L2);
        residual_norm = std::sqrt(du_x_error_L2.numeric_norm_L2 * du_x_error_L2.numeric_norm_L2 +
