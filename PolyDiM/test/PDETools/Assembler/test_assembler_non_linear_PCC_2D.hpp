...
TEST(TEST_assembler_non_linear_PCC_2D, TEST_assembler_non_linear_example)
{
    ...
    auto exact_solution_function = [](const double &x, const double &y, const double &z) {
        return 16.0 * x * y * (1.0 - x) * (1.0 - y);
    };

    auto exact_gradient_solution_function = [](const double &x, const double &y, const double &z) {
        return std::array<double, 3>({16.0 * (1.0 - 2.0 * x) * y * (1.0 - y), 16.0 * (1.0 - 2.0 * y) * x * (1.0 - x), 0.0});
    };

    auto exact_laplacian_solution_function = [](const double &x, const double &y, const double &z) {
    ...
    const auto u_strong = PDETools::Assembler_Utilities::PCC_2D::assemble_strong_solution(geometry_utilities,
                                                                                          mesh,
                                                                                          mesh_geometric_data,
    ...
    auto source_term_function = [&exact_solution_function,
    ...
    auto diffusion_term_function =
        [](const double &x, const double &y, const double &z, const double &u_k, const std::array<double, 3> &u_grad_k) {
            return 1.0;
        };

    auto advection_term_function =
        [](const double &x, const double &y, const double &z, const double &u_k, const std::array<double, 3> &u_grad_k) {
            return std::array<double, 3>({u_k, u_k, 0.0});
        };

    auto reaction_term_function =
        [](const double &x, const double &y, const double &z, const double &u_k, const std::array<double, 3> &u_grad_k) {
            return (u_grad_k.at(0) + u_grad_k.at(1));
        };

    auto adv_non_linear_function =
        [](const double &x, const double &y, const double &z, const double &u_k, const std::array<double, 3> &u_grad_k) {
            return std::array<double, 3>({u_grad_k.at(0), u_grad_k.at(1), 0.0});
        };

    auto rct_non_linear_function =
        [](const double &x, const double &y, const double &z, const double &u_k, const std::array<double, 3> &u_grad_k) {
    ...
    while (num_iteration < max_iterations && residual_norm > newton_tol * solution_norm)
    {
        ...
        const auto adv_source_term =
            PDETools::Assembler_Utilities::PCC_2D::assemble_source_term_gradients(geometry_utilities,
                                                                                  mesh,
                                                                                  mesh_geometric_data,
                                                                                  trial_dofs_data,
                                                                                  test_dofs_data,
    ...
    const auto u_on_cell0Ds =
        PDETools::Assembler_Utilities::PCC_2D::extract_solution_on_cell0Ds(mesh, trial_dofs_data, u_k, u_strong, exact_solution_function, exact_gradient_solution_function);
