                        ...
                        Gedim::MeshMatricesDAO &mesh)
{
    ...
    case Polydim::PDETools::Mesh::PDE_Mesh_Utilities::MeshGenerator_Types_1D::CsvImporter: {
        Polydim::PDETools::Mesh::PDE_Mesh_Utilities::import_mesh_1D(config.MeshGenerator(), config.MeshImportFilePath(), mesh);
    }
    break;
    default:
                     ...
                     const std::string &exportVtuFolder)
{
    ...
    std::string test_type{
        Gedim::io_enum::enum_to_string<Polydim::examples::Elliptic_PCC_1D::test::Test_Types, 1, 4>(config.TestType())};
    std::string mesh_generator{
        Gedim::io_enum::enum_to_string<Polydim::PDETools::Mesh::PDE_Mesh_Utilities::MeshGenerator_Types_1D, 0, 20>(
            config.MeshGenerator())};
    std::string method_type{
        Gedim::io_enum::enum_to_string<Polydim::FEM::PCC::FEM_PCC_1D_LocalSpace_Types, 0, 20>(config.MethodType())};

    {
        ...
        error_to_str << std::setw(w + 10) << "ProgramType" << std::setw(w + 10) << "MethodType" << std::setw(w)
                     << "MethodOrder" << std::setw(w) << "MeshGenerator" << std::setw(w) << "Cell1Ds" << std::setw(w)
        ...
        error_to_str << std::setw(w + 14) << test_type << std::setw(w + 10) << method_type << std::setw(w)
                     << config.MethodOrder() << std::setw(w) << mesh_generator << std::setw(w) << mesh.Cell1DTotalNumber()
    ...
    if (config.ExportFormat()[1])
    {
        ...
        exporter.Export(exportVtuFolder + "/Solution_" + std::to_string(TEST_ID) + "_" + std::to_string(TEST_ID) + "_" +
