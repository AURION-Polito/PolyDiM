  ...
  namespace examples
  {
    namespace Elliptic_PCC_3D
    {
                            ...
                            Elliptic_PCC_3D_Problem_Data &assembler_data) const
      {
        ...
        std::cout<< "DEBUG - c3D "<< cell3DIndex<< std::endl;
        ...
        for (unsigned int p = 0; p < mesh.Cell3DNumberVertices(cell3DIndex); ++p)
        {
          ...
          const auto strong_boundary_values = test.strong_boundary_condition(1, coordinates);
          ...
          for (unsigned int loc_i = 0; loc_i < local_dofs.size(); ++loc_i)
          {
            ...
            switch (local_dof_i.Type)
            {
              case Polydim::PDETools::DOFs::DOFsManager::DOFsData::DOF::Types::Strong:
                std::cout<< std::scientific<< "\tBORDER V "<< cell0D_index;
                std::cout<< " bi "<< static_cast<unsigned int>(boundary_info.Type);
                std::cout<< " bm "<< boundary_info.Marker;
                std::cout<< " ld "<< loc_i<< " gd "<< local_dof_i.Global_Index;
                std::cout<< " value "<< strong_boundary_values[loc_i]<< std::endl;
                break;
              case Polydim::PDETools::DOFs::DOFsManager::DOFsData::DOF::Types::DOF:
                std::cout<< std::scientific<< "\tINTERN V "<< cell0D_index;
                std::cout<< " bi "<< static_cast<unsigned int>(boundary_info.Type);
                std::cout<< " bm "<< boundary_info.Marker;
                std::cout<< " ld "<< loc_i<< " gd "<< local_dof_i.Global_Index;
                std::cout<< " value "<< strong_boundary_values[loc_i]<< std::endl;
        ...
        for (unsigned int e = 0; e < mesh.Cell3DNumberEdges(cell3DIndex); ++e)
        {
          ...
          const auto strong_boundary_values = test.strong_boundary_condition(1, edge_dofs_coordinates);
          ...
          for (unsigned int loc_i = 0; loc_i < local_dofs.size(); ++loc_i)
          {
            ...
            {
              case Polydim::PDETools::DOFs::DOFsManager::DOFsData::DOF::Types::Strong:
                std::cout<< std::scientific<< "\tBORDER E "<< cell1D_index;
                std::cout<< " bi "<< static_cast<unsigned int>(boundary_info.Type);
                std::cout<< " bm "<< boundary_info.Marker;
                std::cout<< " ld "<< loc_i<< " gd "<< local_dof_i.Global_Index;
                std::cout<< " value "<< strong_boundary_values[loc_i]<< std::endl;
                break;
              case Polydim::PDETools::DOFs::DOFsManager::DOFsData::DOF::Types::DOF:
                std::cout<< std::scientific<< "\tINTERN E "<< cell1D_index;
                std::cout<< " bi "<< static_cast<unsigned int>(boundary_info.Type);
                std::cout<< " bm "<< boundary_info.Marker;
                std::cout<< " ld "<< loc_i<< " gd "<< local_dof_i.Global_Index;
                std::cout<< " value "<< strong_boundary_values[loc_i]<< std::endl;
        ...
        for (unsigned int f = 0; f < mesh.Cell3DNumberFaces(cell3DIndex); f++)
        {
          ...
          const Eigen::VectorXd dirichletValues =
              test.strong_boundary_condition(1, face_dofs_coordinates.Points);
          ...
          for (unsigned int loc_i = 0; loc_i < local_dofs.size(); ++loc_i)
          {
            ...
            {
              case Polydim::PDETools::DOFs::DOFsManager::DOFsData::DOF::Types::Strong:
                std::cout<< std::scientific<< "\tBORDER F "<< cell2D_index;
                std::cout<< " bi "<< static_cast<unsigned int>(boundary_info.Type);
                std::cout<< " bm "<< boundary_info.Marker;
                std::cout<< " ld "<< loc_i<< " gd "<< local_dof_i.Global_Index;
                std::cout<< " cd "<< face_dofs_coordinates.Points.col(loc_i).transpose();
                std::cout<< " value "<< strong_boundary_values[loc_i]<< std::endl;
                break;
              case Polydim::PDETools::DOFs::DOFsManager::DOFsData::DOF::Types::DOF:
                std::cout<< std::scientific<< "\tINTERN F "<< cell2D_index;
                std::cout<< " bi "<< static_cast<unsigned int>(boundary_info.Type);
                std::cout<< " bm "<< boundary_info.Marker;
                std::cout<< " ld "<< loc_i<< " gd "<< local_dof_i.Global_Index;
                std::cout<< " cd "<< face_dofs_coordinates.Points.col(loc_i).transpose();
                                      ...
                                      Elliptic_PCC_3D_Problem_Data &assembler_data) const
      {
        ...
        {
          const unsigned int cell2D_index = mesh.Cell3DFace(cell3DIndex, f);
          const auto face_quadrature = local_space::FaceQuadrature(reference_element_data,
                                                                   local_space_data,
                                                                   f,
          ...
          const auto face_basis_function_values =
              local_space::BasisFunctionsValuesOnFace(f,
                                                      reference_element_data,
                                                      local_space_data,
                                                      face_quadrature.Points);

          const Eigen::VectorXd weak_boundary_values =
              face_basis_function_values.transpose() *
              face_quadrature.Weights.asDiagonal() *
              neumannValues;
          ...
          const Polydim::examples::Elliptic_PCC_3D::test::I_Test &test) const
      {
        ...
        for (unsigned int c = 0; c < mesh.Cell3DTotalNumber(); ++c)
        {
          ...
          Debug(c, mesh, mesh_dofs_info, dofs_data, reference_element_data, local_space_data, test, result);
