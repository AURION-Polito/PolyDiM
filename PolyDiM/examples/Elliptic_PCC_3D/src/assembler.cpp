          ...
          const Polydim::examples::Elliptic_PCC_3D::test::I_Test &test) const
      {
        ...
        for (unsigned int c = 0; c < mesh.Cell3DTotalNumber(); ++c)
        {
          ...
          if (true)
          {
            ...
            const auto& fem_local_space = *reference_element_data.FEM_LocalSpace;
            ...
            {
              const double face_normal_sign = mesh_geometric_data.Cell3DsFacesNormalDirections[c][f] ? +1.0 : -1.0;
              const Eigen::Vector3d face_normal = face_normal_sign * mesh_geometric_data.Cell3DsFacesNormals[c][f];
              const auto& boundary_quadrature = fem_local_data.BoundaryQuadrature[f];
              const auto boundary_values = fem_local_space.ComputeBasisFunctionsValues(fem_reference_data,
                                                                                         fem_local_data,
                                                                                         boundary_quadrature.Points);
              const Eigen::VectorXd face_integral = boundary_values.transpose() *
                                boundary_quadrature.Weights *
                                face_normal.sum();
              boundary_integral += face_integral;
            }

            const auto& internal_quadrature = fem_local_data.InternalQuadrature;
            const auto derivative_values = fem_local_space.ComputeBasisFunctionsDerivativeValues(fem_reference_data,
                                                                  fem_local_data,
                                                                  internal_quadrature.Points);

            Eigen::VectorXd internal_integral = Eigen::VectorXd::Zero(fem_local_data.NumberOfBasisFunctions);
            internal_integral += derivative_values[0].transpose() * internal_quadrature.Weights;
            internal_integral += derivative_values[1].transpose() * internal_quadrature.Weights;
            internal_integral += derivative_values[2].transpose() * internal_quadrature.Weights;

            assert((boundary_integral - internal_integral).norm() < 1.0e-15 * boundary_integral.norm());
          }
