    ...
    namespace Elliptic_PCC_3D
    {
          ...
          const Polydim::examples::Elliptic_PCC_3D::test::I_Test &test) const
      {
        ...
        for (unsigned int c = 0; c < mesh.Cell3DTotalNumber(); ++c)
        {
          ...
          if (true)
          {
            const auto& polyedron_faces_normal = mesh_geometric_data.Cell3DsFacesNormals.at(c);
            const auto& polyhedron_faces_normal_direction = mesh_geometric_data.Cell3DsFacesNormalDirections.at(c);
            ...
            for (unsigned int b = 0; b < polyedron_faces_normal.size(); ++b)
            {
              const Eigen::Vector3d boundary_normal = polyhedron_faces_normal_direction[b] ?
                                                        ...
                                                        -1.0 * polyedron_faces_normal[b];
              const auto& boundary_quadrature = fem_local_data.BoundaryQuadrature[b];
            ...
            assert((boundary_integral - internal_integral).norm() < 1.0e-13 * boundary_integral.norm());
          }
