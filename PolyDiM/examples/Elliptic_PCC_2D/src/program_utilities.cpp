                 ...
                 const std::string& exportVtuFolder)
{
    ...
    for (unsigned int c = 0; c < mesh.Cell1DTotalNumber(); ++c)
    {
        ...
        const Eigen::VectorXd local_edge_coordinates = vem_reference_element_data.Quadrature.ReferenceEdgeDOFsInternalPoints.row(0);
    ...
    for (unsigned int c = 0; c < mesh.Cell2DTotalNumber(); ++c)
    {
        ...
        const auto local_polygon_coordinates = geometryUtilities.EquispaceCoordinates(num_loc_dofs + 1,
                                                                                      0.0,
                                                                                      1.0,
                                                                                      true);
        const Eigen::Vector3d polygon_centroid = mesh_geometric_data.Cell2DsCentroids.at(c);
        const auto polygonCentroidEdgesDistance = geometryUtilities.PolygonCentroidEdgesDistance(mesh_geometric_data.Cell2DsVertices.at(c),
