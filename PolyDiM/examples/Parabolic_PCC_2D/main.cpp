...
int main(int argc, char **argv)
{
  ...
  for(unsigned int t = 1; t < time_steps.size(); t++)
  {
    ...
    auto assembler_data =
        assembler.Assemble(config, mesh, meshGeometricData, meshDOFsInfo, dofs_data, reference_element_data, *test, time_value);
    ...
    auto post_process_data =
        assembler.PostProcessSolution(config, mesh, meshGeometricData, dofs_data, reference_element_data, assembler_data, *test, time_value);
