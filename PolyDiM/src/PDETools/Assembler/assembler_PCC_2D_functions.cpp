  ...
  namespace PDETools
  {
    namespace Assembler_Utilities
    {
      namespace PCC_2D
      {
        ...
        // ***************************************************************************
        Eigen::VectorXd function_evaluation(
            ...
            const std::function<double(const double &, const double &, const double &, const double &, const std::array<double, 3> &)> &f)
        {
          ...
          {
            function_values[i] =
                f(points(0, i), points(1, i), points(2, i), u[i], std::array<double, 3>({u_gradient.at(0)[i], u_gradient.at(1)[i], 0.0}));
          }

          return function_values;
        }
        // ***************************************************************************
        std::array<Eigen::VectorXd, 3> function_evaluation(
            const Eigen::MatrixXd &points,
            const Eigen::VectorXd &u,
            const std::array<Eigen::VectorXd, 2> &u_gradient,
                                             ...
                                             const std::function<double(const double &, const double &, const double &)> &source_term_function)
        {
          ...
          for (unsigned int c = 0; c < mesh.Cell2DTotalNumber(); ++c)
          {
            ...
            const auto test_local_space_data =
                Polydim::PDETools::LocalSpace_PCC_2D::CreateLocalSpace(geometry_utilities.Tolerance1D(),
                                                                       geometry_utilities.Tolerance2D(),
                                                                       mesh_geometric_data,
                                                                       c,
        ...
        // ***************************************************************************
        Variational_Operator assemble_elliptic_operator(
            ...
            const std::function<double(const double &, const double &, const double &)> &reaction_term_function)
        {
          ...
          elliptic_matrix.SetSize(test_dofs_data.NumberDOFs, trial_dofs_data.NumberDOFs, Gedim::ISparseArray::SparseArrayTypes::None);
          elliptic_strong_matrix.SetSize(test_dofs_data.NumberDOFs, trial_dofs_data.NumberStrongs);
          ...
          for (unsigned int c = 0; c < mesh.Cell2DTotalNumber(); ++c)
          {
            ...
            const auto test_local_space_size =
                Polydim::PDETools::LocalSpace_PCC_2D::Size(test_reference_element_data, test_local_space_data);
            const auto trial_local_space_size =
                Polydim::PDETools::LocalSpace_PCC_2D::Size(trial_reference_element_data, trial_local_space_data);

            Eigen::MatrixXd local_A = Eigen::MatrixXd::Zero(test_local_space_size, trial_local_space_size);
            ...
            {
              const auto advection_term_values = function_evaluation(cell2D_internal_quadrature.Points, advection_term_function);
            ...
            assert(trial_local_space_size == trial_dofs_data.CellsGlobalDOFs[2].at(c).size());
            assert(test_local_space_size == test_dofs_data.CellsGlobalDOFs[2].at(c).size());
        ...
        // ***************************************************************************
        Evaluate_Function_On_DOFs_Data evaluate_function_on_dofs(
            const Gedim::GeometryUtilities &geometry_utilities,
            const Gedim::MeshMatricesDAO &mesh,
            const Gedim::MeshUtilities::MeshGeometricData2D &mesh_geometric_data,
            const DOFs::DOFsManager::DOFsData &trial_dofs_data,
          ...
          for (unsigned int c = 0; c < mesh.Cell2DTotalNumber(); ++c)
          {
            ...
            for (unsigned int ed = 0; ed < numVertices; ed++)
            {
              ...
              const auto weak_basis_function_values =
                  Polydim::PDETools::LocalSpace_PCC_2D::BasisFunctionsValuesOnEdge(ed,
                                                                                   test_reference_element_data,
                                                                                   test_local_space_data,
        ...
        // ***************************************************************************
        Post_Process_Data_Cell0Ds extract_solution_on_cell0Ds(
            const Gedim::MeshMatricesDAO &mesh,
            const DOFs::DOFsManager::DOFsData &trial_dofs_data,
                                                   ...
                                                   const std::function<double(const double &, const double &, const double &)> &exact_solution_function)
        {
          ...
          for (unsigned int c = 0; c < mesh.Cell2DTotalNumber(); c++)
          {
            ...
            const Eigen::VectorXd local_error_L2 =
                (trial_basis_functions_values * dofs_values - exact_solution_values).array().square();
                                                   ...
                                                   const std::function<std::array<double, 3>(const double &, const double &, const double &)> &exact_gradient_solution_function)
        {
          ...
          for (unsigned int c = 0; c < mesh.Cell2DTotalNumber(); c++)
          {
            ...
            if (exact_gradient_solution_function)
              exact_derivative_solution_values =
                  function_evaluation(cell2D_internal_quadrature.Points, exact_gradient_solution_function);
            else
            {
            ...
            const Eigen::VectorXd local_error_H1 =
                (trial_basis_functions_derivative_values.at(0) * dofs_values - exact_derivative_solution_values.at(0))
                .array()
                .square() +
                (trial_basis_functions_derivative_values.at(1) * dofs_values - exact_derivative_solution_values.at(1))
                .array()
                .square();
            ...
            const std::function<std::array<double, 3>(const double &, const double &, const double &)> &exact_gradient_solution_function)
        {
          ...
          for (unsigned int c = 0; c < mesh.Cell2DTotalNumber(); c++)
          {
            ...
            result.quadrature_points.block(0, local_num_q, 3, cell2D_num_q) = quadrature_points.at(c);
            result.quadrature_weigths.segment(local_num_q, cell2D_num_q) = quadrature_weigths.at(c);
            result.numeric_solution.segment(local_num_q, cell2D_num_q) = numeric_solution.at(c);
            result.exact_solution.segment(local_num_q, cell2D_num_q) = exact_solution.at(c);
            result.numeric_gradient_solution.at(0).segment(local_num_q, cell2D_num_q) = numeric_gradient_solution.at(c).at(0);
            result.numeric_gradient_solution.at(1).segment(local_num_q, cell2D_num_q) = numeric_gradient_solution.at(c).at(1);
            result.numeric_gradient_solution.at(2).segment(local_num_q, cell2D_num_q) = numeric_gradient_solution.at(c).at(2);
            result.exact_gradient_solution.at(0).segment(local_num_q, cell2D_num_q) = exact_gradient_solution.at(c).at(0);
            result.exact_gradient_solution.at(1).segment(local_num_q, cell2D_num_q) = exact_gradient_solution.at(c).at(1);
            result.exact_gradient_solution.at(2).segment(local_num_q, cell2D_num_q) = exact_gradient_solution.at(c).at(2);
            ...
            const std::function<double(const double &, const double &, const double &, const double &, const std::array<double, 3> &)> &source_term_function)
        {
          ...
          for (unsigned int c = 0; c < mesh.Cell2DTotalNumber(); ++c)
          {
            ...
            const auto source_term_values =
                function_evaluation(cell2D_internal_quadrature.Points, u_on_quadrature, u_gradient_on_quadrature, source_term_function);
          ...
          for (unsigned int c = 0; c < mesh.Cell2DTotalNumber(); ++c)
          {
            ...
            if (diffusion_term_function)
            {
              const auto diffusion_term_values =
                  function_evaluation(cell2D_internal_quadrature.Points, u_on_quadrature, u_gradient_on_quadrature, diffusion_term_function);
            ...
            if (advection_term_function)
            {
              const auto advection_term_values =
                  function_evaluation(cell2D_internal_quadrature.Points, u_on_quadrature, u_gradient_on_quadrature, advection_term_function);
            ...
            if (reaction_term_function)
            {
              const auto reaction_term_values =
                  function_evaluation(cell2D_internal_quadrature.Points, u_on_quadrature, u_gradient_on_quadrature, reaction_term_function);
                                                  ...
                                                  const Eigen::VectorXd &quadrature_weights)
        {
          ...
          {
            for (unsigned int d2 = 0; d2 < 2; d2++)
              cellRightHandSide.noalias() += basis_functions_values.at(d1).transpose() *
        ...
        // ***************************************************************************
        Polydim::PDETools::Assembler_Utilities::PCC_2D::NS_Operators assemble_NS_operators(const Gedim::GeometryUtilities &geometry_utilities,
                                                                                           const Gedim::MeshMatricesDAO &mesh,
                                                                                           const Gedim::MeshUtilities::MeshGeometricData2D &mesh_geometric_data,
        ...
        {
          std::vector<Polydim::PDETools::DOFs::DOFsManager::DOFsData> dofs_data = {
            speed_component_dofs_data,
            speed_component_dofs_data
          };
          ...
          convective_matrix.SetSize(count_dofs.num_total_dofs,
                                    count_dofs.num_total_dofs,
                                    Gedim::ISparseArray::SparseArrayTypes::None);
          convective_strong_matrix.SetSize(count_dofs.num_total_dofs,
                                           count_dofs.num_total_strong);
          ...
          numerical_speed_solution.segment(speed_component_dofs_data.NumberDOFs,
                                           speed_component_dofs_data.NumberDOFs) = numerical_speed_y_solution;
          ...
          numerical_speed_solution_strong.segment(speed_component_dofs_data.NumberStrongs,
                                                  speed_component_dofs_data.NumberStrongs) = numerical_speed_y_solution_strong;
          ...
          for (unsigned int c = 0; c < mesh.Cell2DTotalNumber(); ++c)
          {
            ...
            const auto speed_component_local_space_size =
                Polydim::PDETools::LocalSpace_PCC_2D::Size(speed_component_reference_element_data,
                                                           speed_component_local_space_data);

            std::array<Eigen::MatrixXd, 2> speed_basis_functions_values;
            speed_basis_functions_values.at(0).setZero(speed_component_basis_functions_values.rows(),
                                                       2 * speed_component_local_space_size);
            speed_basis_functions_values.at(0).middleCols(0,
                                                          speed_component_local_space_size) = speed_component_basis_functions_values;
            speed_basis_functions_values.at(1).setZero(speed_component_basis_functions_values.rows(),
                                                       2 * speed_component_local_space_size);
            speed_basis_functions_values.at(1).middleCols(speed_component_local_space_size,
                                                          speed_component_local_space_size) = speed_component_basis_functions_values;

            std::array<Eigen::MatrixXd, 4> speed_basis_functions_derivative_values;
            for (unsigned int d = 0; d < 4; ++d)
            {
              speed_basis_functions_derivative_values.at(d).setZero(speed_component_basis_functions_derivative_values.at(0).rows(),
                                                                    2 * speed_component_local_space_size);
            }
            for (unsigned int d1 = 0; d1 < 2; d1++)
            {
              for (unsigned int d2 = 0; d2 < 2; d2++)
              {
                speed_basis_functions_derivative_values.at(2 * d1 + d2).middleCols(d1 * speed_component_local_space_size,
                                                                                   speed_component_local_space_size) =
            ...
            u_gradient_on_quadrature.at(0) = speed_basis_functions_derivative_values.at(0) *
                                             speed_dofs_values;
            u_gradient_on_quadrature.at(1) = speed_basis_functions_derivative_values.at(1) *
                                             speed_dofs_values;
            u_gradient_on_quadrature.at(2) = speed_basis_functions_derivative_values.at(2) *
                                             speed_dofs_values;
            u_gradient_on_quadrature.at(3) = speed_basis_functions_derivative_values.at(3) *
                                             speed_dofs_values;
          ...
          return  {
            convert_operator(convective_matrix, convective_strong_matrix),
                static_cast<Eigen::VectorXd &>(convective_rhs_term)
          };
        }
        // ***************************************************************************
      } // namespace PCC_2D
