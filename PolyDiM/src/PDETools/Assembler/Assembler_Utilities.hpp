...
namespace Polydim
{
  namespace PDETools
  {
    namespace Assembler_Utilities
    {
      ...
      void assemble_local_matrix_to_global_matrix(
          ...
          global_rhs_type& global_rhs)
      {
        for(unsigned test_f = 0; test_f < test_functions_dofs_data.dofs_data.size(); ++test_f)
        {
          ...
          for (unsigned int test_loc_i = 0; test_loc_i < test_global_dofs.size(); ++test_loc_i)
          {
            ...
            const auto local_dof_i = test_dofs_data.CellsDOFs.at(global_dof_i.Dimension).at(global_dof_i.CellIndex).at(global_dof_i.DOFIndex);
            ...
            const unsigned int global_index_i = local_dof_i.Global_Index +
                                                test_global_offset_DOFs;

            global_rhs.AddValue(global_index_i,
                                local_rhs[test_loc_i + test_local_offset]);
            ...
            for(unsigned int trial_f = 0; trial_f < trial_functions_dofs_data.dofs_data.size(); ++trial_f)
            {
              ...
              for (unsigned int trial_loc_j = 0; trial_loc_j < trial_global_dofs.size(); ++trial_loc_j)
              {
                ...
                const auto& local_dof_j = trial_dofs_data.CellsDOFs.at(global_dof_j.Dimension).at(global_dof_j.CellIndex).at(global_dof_j.DOFIndex);

                const unsigned int global_index_j = local_dof_j.Global_Index;
                const double loc_A_element =  local_lhs(test_loc_i +
                                                        test_local_offset,
                                                        trial_loc_j +
                                                        trial_local_offset);
                ...
                {
                  case Polydim::PDETools::DOFs::DOFsManager::DOFsData::DOF::Types::Strong:
                    global_lhs_Strongs.Triplet(global_index_i,
                                               global_index_j +
                                               trial_global_offset_Strongs,
                                               loc_A_element);
                    break;
                  case Polydim::PDETools::DOFs::DOFsManager::DOFsData::DOF::Types::DOF:
                    global_lhs_DOFs.Triplet(global_index_i,
                                            global_index_j +
                                            trial_global_offset_DOFs,
                                            loc_A_element);
                    break;
                  default:
                    throw std::runtime_error("Unknown DOF Type");
