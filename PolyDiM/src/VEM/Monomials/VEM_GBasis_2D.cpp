...
VEM_GBasis_Data VEM_GBasis_2D::Compute(const unsigned int polynomial_degree)
{
    ...
    for (unsigned int j = 0; j < data.Nk; j++)
    {
        ...
        if(expo(1) > 0)
            data.VectorDecomposition[0][1](j, VectorDecompositionIndex[0](1)) = ((double)expo(1)) / (data.monomials_data.Exponents[j].sum() + 1);

        data.VectorDecomposition[1][0](j, VectorDecompositionIndex[1](0)) = 1.0 / (data.monomials_data.Exponents[j].sum() + 1);
        if(expo(0) > 0)
            data.VectorDecomposition[1][1](j, VectorDecompositionIndex[1](1)) =  - ((double)expo(0)) / (data.monomials_data.Exponents[j].sum() + 1);
...
//****************************************************************************
vector<Vector2i> VEM_GBasis_2D::VectorDecompositionIndices(const VEM_GBasis_Data& data,
