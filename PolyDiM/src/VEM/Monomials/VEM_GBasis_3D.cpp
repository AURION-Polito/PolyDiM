...
VEM_GBasis_Data VEM_GBasis_3D::Compute(const unsigned int polynomial_degree)
{
    ...
    data.DimFirstBasis = data.Nkm1 - (polynomial_degree - 1)
                                         * polynomial_degree
    ...
    for (unsigned int i=1; i < data.Nkp1; i++)
    {
        if(data.monomials_data.Exponents[i-1](0)==0)
        {
            if(data.monomials_data.Exponents[i-1](1)==0)
                ...
                data.MapExponents[data.monomials_data.Exponents[i-1](0)+data.monomials_data.Exponents[i-1](1)-1][0][data.monomials_data.Exponents[i-1](2)+1] = i;
        }
        else
            data.MapExponents[data.monomials_data.Exponents[i-1](0)-1][data.monomials_data.Exponents[i-1](1)+1][data.monomials_data.Exponents[i-1](2)] = i;
    ...
    {
        const VectorXi& expo = data.monomials_data.Exponents[j];
        const vector<Vector4i> VectorDecompositionIndex = VectorDecompositionIndices(data,
                                                                                     expo);
...
//****************************************************************************
vector<Vector4i> VEM_GBasis_3D::VectorDecompositionIndices(const VEM_GBasis_Data& data,
                                                           const VectorXi& expo) const
{
    ...
    vectorDecompositionIndices[1](2) = (expo(0) > 0 && expo(2) > 0) ? data.MapExponents[expo(0) - 1][expo(1) + 1][expo(2) - 1]: -1;
    ...
    vectorDecompositionIndices[2](3) = (expo(0) > 0 && expo(1) > 0) ? data.MapExponents[expo(0) - 1][expo(1) - 1][expo(2) + 1]: -1;

    return vectorDecompositionIndices;
}
//****************************************************************************
vector<MatrixXd> VEM_GBasis_3D::VanderGBigOPlus(const VEM_GBasis_Data& data,
                                                const MatrixXd& vander) const
{
