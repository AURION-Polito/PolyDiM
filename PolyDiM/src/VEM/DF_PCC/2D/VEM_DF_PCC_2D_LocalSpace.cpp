#include "VEM_DF_PCC_2D_LocalSpace.hpp"
...
namespace DF_PCC {
//****************************************************************************
VEM_DF_PCC_2D_LocalSpace_Data VEM_DF_PCC_2D_LocalSpace::CreateLocalSpace(const VEM_DF_PCC_2D_Velocity_ReferenceElement_Data &reference_element_data,
                                                                         const VEM_DF_PCC_2D_Polygon_Geometry &polygon) const
{
    VEM_DF_PCC_2D_LocalSpace_Data localSpace;
    ...
    localSpace.EdgesDOFs = quadrature.PolygonEdgesLobattoQuadrature(reference_element_data.Quadrature.ReferenceEdgeDOFsInternalPoints,
    ...
    ComputePolynomialBasisDofs(localSpace.InternalQuadrature.Weights,
                               localSpace);

    ComputeDivergenceCoefficients(localSpace.EdgesDOFs.WeightsTimesNormal,
                                  localSpace);

    ComputeCMatrixkm2(polygon.Diameter,
...
//****************************************************************************
void VEM_DF_PCC_2D_LocalSpace::InitializeProjectorsComputation(const VEM_DF_PCC_2D_Velocity_ReferenceElement_Data &reference_element_data,
                                                               ...
                                                               VEM_DF_PCC_2D_LocalSpace_Data &localSpace) const
{
    ...
    localSpace.VanderGBigOPluskm2.resize(localSpace.Dimension, MatrixXd::Zero(localSpace.VanderInternal.rows(), localSpace.NumBigOPlusInternalBasisFunctions));
    ...
    localSpace.VanderKp1EdgeProjections.block(0,
                                              0,
                                              localSpace.NumVertexBasisFunctions,
                                              localSpace.NumVertexBasisFunctions)
        = Eigen::MatrixXd::Identity(localSpace.NumVertexBasisFunctions,
                                    localSpace.NumVertexBasisFunctions);

    localSpace.EdgeBasisCoefficients = utilities.ComputeEdgeBasisCoefficients(reference_element_data.Order,
                                                                              localSpace.ReferenceEdgeDofInternalPoints);
    ...
    for (unsigned int e = 0; e < numEdges; e++)
    {
        ...
        localSpace.VanderKp1EdgeProjections.block(edgeInternalPointsOffset,
                                                  ...
                                                  1) = edgeFunctionValues.col(0);
        localSpace.VanderKp1EdgeProjections.block(edgeInternalPointsOffset,
                                                  edgeEnd,
                                                  numEdgeInternalQuadraturePoints,
                                                  1) = edgeFunctionValues.col(1);
        localSpace.VanderKp1EdgeProjections.block(edgeInternalPointsOffset,
...
//****************************************************************************
void VEM_DF_PCC_2D_LocalSpace::ComputeDivergenceCoefficients(const std::vector<Eigen::VectorXd>& boundaryDofQuadratureWeightsTimesNormal,
                                                             VEM_DF_PCC_2D_LocalSpace_Data& localSpace) const
{
    localSpace.Wmatrix = MatrixXd::Zero(localSpace.Nkm1,
    ...
    localSpace.Wmatrix.block(1, localSpace.NumBoundaryBasisFunctions + localSpace.NumBigOPlusInternalBasisFunctions,
        ...
        = MatrixXd::Identity(localSpace.NumDivergenceInternalBasisFunctions,
                             localSpace.NumDivergenceInternalBasisFunctions);


    localSpace.Wmatrix.block(0, 0,
                             1, localSpace.NumVertexBasisFunctions + localSpace.NumEdgeBasisFunctions)
        = boundaryDofQuadratureWeightsTimesNormal[0].transpose();

    localSpace.Wmatrix.block(0, localSpace.NumVertexBasisFunctions + localSpace.NumEdgeBasisFunctions,
                             1, localSpace.NumVertexBasisFunctions + localSpace.NumEdgeBasisFunctions)
...
//****************************************************************************
void VEM_DF_PCC_2D_LocalSpace::ComputePolynomialBasisDofs(const Eigen::VectorXd& internalQuadratureWeights,
                                                          VEM_DF_PCC_2D_LocalSpace_Data& localSpace) const
{
    ...
    for(unsigned int d = 0; d < localSpace.Dimension; d++)
    {
        ...
        localSpace.Dmatrix[d].block(d * (localSpace.NumVertexBasisFunctions + localSpace.NumEdgeBasisFunctions), 0,
                                    localSpace.NumVertexBasisFunctions + localSpace.NumEdgeBasisFunctions, localSpace.Nk)
            = localSpace.VanderBoundary;

        // Big o plus
        localSpace.Dmatrix[d].middleRows(localSpace.NumBoundaryBasisFunctions,
                                         localSpace.NumBigOPlusInternalBasisFunctions)
            = localSpace.VanderGBigOPluskm2[d].transpose()
...
//****************************************************************************
void VEM_DF_PCC_2D_LocalSpace::ComputeStabilizationMatrix(VEM_DF_PCC_2D_LocalSpace_Data& localSpace) const
{
...
//****************************************************************************
void VEM_DF_PCC_2D_LocalSpace::ComputeCMatrixkm2(const double& polygonDiameter,
                                                 ...
                                                 VEM_DF_PCC_2D_LocalSpace_Data& localSpace) const
{
    ...
    // DOF divergenza
    localSpace.Cmatrix[0] += - polygonDiameter * localSpace.VectorDecompositionMatrices[0][0].block(0, 1, localSpace.Nk, localSpace.NKp1 - 1)
                             * localSpace.HmatrixKp1.block(1, 0, localSpace.NKp1 - 1, localSpace.Nkm1) * localSpace.Vmatrix;

    localSpace.Cmatrix[1] += - polygonDiameter * localSpace.VectorDecompositionMatrices[1][0].block(0, 1, localSpace.Nk, localSpace.NKp1 - 1)
                             * localSpace.HmatrixKp1.block(1, 0, localSpace.NKp1 - 1, localSpace.Nkm1) * localSpace.Vmatrix;
...
//****************************************************************************
void VEM_DF_PCC_2D_LocalSpace::ComputeL2Projectors(const Eigen::VectorXd& internalQuadratureWeights,
                                                   VEM_DF_PCC_2D_LocalSpace_Data& localSpace) const
{
    // DOF big o plus
    ...
    for(unsigned int d = 0; d < localSpace.Dimension; d++)
        localSpace.Cmatrix[d].bottomRows(localSpace.Nk - localSpace.Nkm2) +=
            localSpace.VectorDecompositionMatrices[d][1].block(localSpace.Nkm2, localSpace.Nkm3,
                                                               localSpace.Nk - localSpace.Nkm2, localSpace.Nkm1 - localSpace.Nkm3) * tmpE;
...
//****************************************************************************
void VEM_DF_PCC_2D_LocalSpace::ComputePiNabla(const VEM_DF_PCC_2D_Velocity_ReferenceElement_Data &reference_element_data,
                                              ...
                                              VEM_DF_PCC_2D_LocalSpace_Data& localSpace) const
{
    ...
    localSpace.Gmatrix.row(0) = ( 1.0 / polygonMeasure) * localSpace.VanderInternal.transpose() *
                                internalQuadratureWeights;

    // B_{ij} = \int_E \nabla m_i \nabla \phi_j
    const MatrixXd BmatrixX = localSpace.VanderBoundaryDerivatives[0].transpose() *
                                  boundaryDofQuadratureWeightsTimesNormal[0].asDiagonal() +
                              localSpace.VanderBoundaryDerivatives[1].transpose() *
                                  boundaryDofQuadratureWeightsTimesNormal[1].asDiagonal();
...
//****************************************************************************
void VEM_DF_PCC_2D_LocalSpace::ComputeL2ProjectorsOfDerivatives(const VEM_DF_PCC_2D_Velocity_ReferenceElement_Data &reference_element_data,
                                                                const double &polygonDiameter,
                                                                const std::vector<Eigen::VectorXd>& boundaryDofQuadratureWeightsTimesNormal,
                                                                VEM_DF_PCC_2D_LocalSpace_Data& localSpace) const
{
    ...
    for(unsigned int d1 = 0; d1 < localSpace.Dimension; d1++)
    {
        for(unsigned int d2 = 0; d2 < localSpace.Dimension; d2++)
        {
            ...
            localSpace.Ematrix[localSpace.Dimension * d1 + d2].middleCols(d1 * (localSpace.NumVertexBasisFunctions + localSpace.NumEdgeBasisFunctions),
                                                                          localSpace.NumVertexBasisFunctions + localSpace.NumEdgeBasisFunctions) +=
                localSpace.VanderBoundary.leftCols(localSpace.Nkm1).transpose() * boundaryDofQuadratureWeightsTimesNormal[d2].asDiagonal();
        }
    }
