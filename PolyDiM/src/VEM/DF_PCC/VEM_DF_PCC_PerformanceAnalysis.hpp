...
struct VEM_DF_PCC_PerformanceAnalysis final
{
                                                ...
                                                const VEM_vem_local_space_dataData_Type& vem_local_space_data) const
    {
        ...
        const Eigen::MatrixXd identity = Eigen::MatrixXd::Identity(vem_local_space_data.Nk,
                                                                   vem_local_space_data.Nk);
        ...
        for(unsigned int d1 = 0; d1 < vem_local_space_data.Dimension; d1++)
        {
            ...
            for(unsigned int d2 = 0; d2 < vem_local_space_data.Dimension; d2++)
            {
                ...
                const Eigen::MatrixXd derMatrix = invDiameter *
                                                  ...
                                                  topLeftCorner(vem_local_space_data.Nk,
                ...
                result.ErrorPi0km1Grad[vem_local_space_data.Dimension * d1 + d2]
                    = (vem_local_space_data.Pi0km1Der[vem_local_space_data.Dimension * d1 + d2] * vem_local_space_data.Dmatrix[d1]
                       - derMatrix.transpose()).norm() / derMatrix.norm();
            }
        }
        ...
        if (vem_local_space_data.StabMatrix.size() > 0)
        {
            ...
            Eigen::MatrixXd polynomialBasisDofs = Eigen::MatrixXd::Zero(vem_local_space_data.NumBasisFunctions, vem_local_space_data.Dimension * vem_local_space_data.Nk);
            for(unsigned int d = 0; d < vem_local_space_data.Dimension; d++)
                polynomialBasisDofs.middleCols(d * vem_local_space_data.Nk, vem_local_space_data.Nk) = vem_local_space_data.Dmatrix[d];
        ...
        if (vem_local_space_data.Hmatrix.size() > 0 && vem_local_space_data.Cmatrix.size() > 0)
        {
            ...
            for(unsigned int d1 = 0; d1 < vem_local_space_data.Dimension; d1++)
                result.ErrorHCD[d1] = (vem_local_space_data.Hmatrix - vem_local_space_data.Cmatrix[d1] * vem_local_space_data.Dmatrix[d1]).norm() * HMatrixInvNorm;
        }
        ...
        if (vem_local_space_data.Gmatrix.size() > 0 && vem_local_space_data.Bmatrix.size() > 0)
        {
            ...
            for(unsigned int d1 = 0; d1 < vem_local_space_data.Dimension; d1++)
                result.ErrorGBD[d1] = (vem_local_space_data.Gmatrix - vem_local_space_data.Bmatrix[d1] * vem_local_space_data.Dmatrix[d1]).norm() * GMatrixInvNorm;
        }
            ...
            for(unsigned int d1 = 0; d1 < vem_local_space_data.Dimension; d1++)
            {
                for(unsigned int d2 = 0; d2 < vem_local_space_data.Dimension; d2++)
                {
                    ...
                    result.ErrorHED[vem_local_space_data.Dimension * d1 + d2]
                        = (vem_local_space_data.Ematrix[vem_local_space_data.Dimension * d1 + d2] * vem_local_space_data.Dmatrix[d1] - derMatrix.transpose()).norm() / derMatrix.norm();
