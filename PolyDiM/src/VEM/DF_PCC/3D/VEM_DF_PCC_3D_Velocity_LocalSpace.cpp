                                                                                           ...
                                                                                           const VEM_DF_PCC_3D_Polyhedron_Geometry& polyhedron) const
{
    ...
    {
        localSpace.facesLocalSpace[f] = faceVemValues.Compute3DUtilities_DF_PCC(reference_element_data_2D,
                                                                                polygonalFaces[f]);
    ...
    ComputeCMatrixkm2(polyhedron.Measure,
                      polyhedron.Diameter,
                      localSpace.BoundaryQuadratureKL.Quadrature.Weights,
                      localSpace);
    ...
    ComputeL2Projectors(localSpace.InternalQuadrature.Weights,
                        localSpace);
...
//****************************************************************************
void VEM_DF_PCC_3D_Velocity_LocalSpace::InitializeProjectorsComputation(const VEM_DF_PCC_3D_Velocity_ReferenceElement_Data &reference_element_data,
                                                                        ...
                                                                        VEM_DF_PCC_3D_Velocity_LocalSpace_Data &localSpace) const
{
    ...
    localSpace.NumBoundaryBasisFunctions = localSpace.Dimension * (localSpace.NumVertexBasisFunctions + localSpace.NumEdgeBasisFunctions)
    ...
    localSpace.NKp1 = (reference_element_data.Order + 3) * (reference_element_data.Order + 2) * (reference_element_data.Order + 4) / 6;
    localSpace.Nk = (reference_element_data.Order + 1) * (reference_element_data.Order + 2) * (reference_element_data.Order + 3) / 6;
    ...
    localSpace.Nkm4 = (reference_element_data.Order - 3) * (reference_element_data.Order - 2) * (reference_element_data.Order - 1) / 6;


    // Compute Vandermonde matrices.
    const MatrixXd vanderInternalKp1 = monomials.Vander(reference_element_data.GBasis.monomials_data,
...
//****************************************************************************
void VEM_DF_PCC_3D_Velocity_LocalSpace::ComputeFaceProjectors(const PCC::VEM_PCC_2D_LocalSpace& faceVemValues,
                                                              ...
                                                              VEM_DF_PCC_3D_Velocity_LocalSpace_Data& localSpace) const
{
    ...
    localSpace.VanderFaceProjectionsKm1.resize(localSpace.Dimension, MatrixXd::Zero(numFaceInternalQuadrature, localSpace.NumBoundaryBasisFunctions));
    localSpace.ScaledHmatrixOnBoundary.resize(localSpace.Dimension, MatrixXd::Zero(numFaceInternalQuadrature, localSpace.NumTangentsBasisFunctions
                                                                                                                  + localSpace.NumNormalBasisFunctions));
    ...
    for(unsigned int numFace = 0; numFace < numFaces; numFace++)
    {
        ...
        const MatrixXd facePolynomialBasisValues = (1.0 / polygonalFaces[numFace].Measure) *
        ...
        const MatrixXd thisFaceProjectedBasisFunctionsValuesKL = faceVemValues.ComputeBasisFunctionsValues(facesLocalSpace[numFace],
                                                                                                           PCC::ProjectionTypes::Pi0klm1);

        const MatrixXd thisFaceProjectedBasisFunctionsValues = faceVemValues.ComputeBasisFunctionsValues(facesLocalSpace[numFace],
                                                                                                         PCC::ProjectionTypes::Pi0km1);
        ...
        for(unsigned int numFaceVertex = 0; numFaceVertex < polyhedronFaces[numFace].cols(); numFaceVertex++)
        {
            for(unsigned int d = 0; d < localSpace.Dimension; d++)
            {
                localSpace.VanderFaceProjectionsKp1TimesNormal.col(d * numVerteXEdgeBasisFunctions + polyhedronFaces[numFace](0, numFaceVertex))
                    .segment(faceQuadraturePointsOffsetKL,
                ...
                localSpace.VanderFaceProjectionsKm1[d].col(d * numVerteXEdgeBasisFunctions + polyhedronFaces[numFace](0, numFaceVertex))
        ...
        for(unsigned int numFaceEdge = 0; numFaceEdge < polyhedronFaces[numFace].cols(); numFaceEdge++)
        {
            ...
            for(unsigned int d = 0; d < localSpace.Dimension; d++)
            {
                localSpace.VanderFaceProjectionsKp1TimesNormal.block(faceQuadraturePointsOffsetKL,
                                                                     d * numVerteXEdgeBasisFunctions + localSpace.NumVertexBasisFunctions +
                                                                         (localSpace.Order - 1) *
                                                                             polyhedronFaces[numFace](1, numFaceEdge),
                                                                     numQuadraturePointsOnFaceKL, (localSpace.Order - 1))
                    = thisFaceProjectedBasisFunctionsValuesKL.block(0, facesLocalSpace[numFace].NumVertexBasisFunctions
                ...
                localSpace.VanderFaceProjectionsKm1[d].block(faceQuadraturePointsOffset,
                                                             d * numVerteXEdgeBasisFunctions + localSpace.NumVertexBasisFunctions +
                                                                 (localSpace.Order - 1) *
                                                                     polyhedronFaces[numFace](1, numFaceEdge),
                                                             numQuadraturePointsOnFace, (localSpace.Order - 1))
                    = thisFaceProjectedBasisFunctionsValues.block(0, facesLocalSpace[numFace].NumVertexBasisFunctions
                                                                         + (localSpace.Order - 1) * numFaceEdge,
                                                                  numQuadraturePointsOnFace,
                                                                  (localSpace.Order - 1));
            }
        ...
        localSpace.VanderFaceProjectionsKp1TimesNormal.block(faceQuadraturePointsOffsetKL, faceNormalDofsOffset,
                                                             numQuadraturePointsOnFaceKL, Nkm2_2D) =
            faceNormalGlobalDirection * thisFaceProjectedBasisFunctionsValuesKL.rightCols(Nkm2_2D);

        for(unsigned int d = 0; d < localSpace.Dimension; d++)
        {
            localSpace.VanderFaceProjectionsKm1[d].block(faceQuadraturePointsOffset, faceNormalDofsOffset,
                                                         numQuadraturePointsOnFace, Nkm2_2D) =
                faceNormalGlobalDirection * thisFaceProjectedBasisFunctionsValues.rightCols(Nkm2_2D)
                * faceNormalDirection * facesNormals[numFace](d);

            localSpace.VanderFaceProjectionsKm1[d].block(faceQuadraturePointsOffset, faceTangent1DofsOffset,
                                                         numQuadraturePointsOnFace, Nkm2_2D) =
                thisFaceProjectedBasisFunctionsValues.rightCols(Nkm2_2D) * faceTangent1GlobalDirection * facesTangents[numFace][0](d);

            localSpace.VanderFaceProjectionsKm1[d].block(faceQuadraturePointsOffset, faceTangent2DofsOffset,
                                                         numQuadraturePointsOnFace, Nkm2_2D) =
                thisFaceProjectedBasisFunctionsValues.rightCols(Nkm2_2D) * faceTangent2GlobalDirection * facesTangents[numFace][1](d);


            localSpace.ScaledHmatrixOnBoundary[d].block(faceQuadraturePointsOffset,
                                                        faceNormalDofsOffset - 3 * numVerteXEdgeBasisFunctions,
                                                        numQuadraturePointsOnFace, Nkm2_2D) =
                faceNormalGlobalDirection * facePolynomialBasisValues
                * faceNormalDirection * facesNormals[numFace](d);
...
//****************************************************************************
void VEM_DF_PCC_3D_Velocity_LocalSpace::ComputeDivergenceCoefficients(const VEM_DF_PCC_3D_Velocity_ReferenceElement_Data &reference_element_data,
                                                                      ...
                                                                      VEM_DF_PCC_3D_Velocity_LocalSpace_Data& localSpace) const
{
    ...
    localSpace.Wmatrix.block(1, localSpace.NumBoundaryBasisFunctions + localSpace.NumBigOPlusInternalBasisFunctions,
                             localSpace.NumDivergenceInternalBasisFunctions, localSpace.NumDivergenceInternalBasisFunctions)
        = (polyhedronMeasure / polyhedronDiameter) * MatrixXd::Identity(localSpace.NumDivergenceInternalBasisFunctions,
                                                                        localSpace.NumDivergenceInternalBasisFunctions);

    const unsigned int Nkm2_2D = reference_element_data.NumDofs2D;
    unsigned int offsetDof = 3 * (localSpace.NumVertexBasisFunctions + localSpace.NumEdgeBasisFunctions)
                             + localSpace.NumTangentsBasisFunctions;
    for(unsigned int f = 0; f < faceNormalGlobalDirections.size(); f++)
    {
        localSpace.Wmatrix(0, offsetDof) = faceNormalGlobalDirections[f] ? polygonalFaces[f].Measure: -polygonalFaces[f].Measure;
        offsetDof += Nkm2_2D;
    }
...
//****************************************************************************
void VEM_DF_PCC_3D_Velocity_LocalSpace::ComputePolynomialBasisDofs(const double& polyhedronMeasure,
                                                                   const double& polyhedronDiameter,
                                                                   const Eigen::VectorXd& internalQuadratureWeights,
                                                                   ...
                                                                   VEM_DF_PCC_3D_Velocity_LocalSpace_Data& localSpace) const
{
    ...
    for(unsigned int d = 0; d < localSpace.Dimension; d++)
    {
        ...
        // Big o plus
        localSpace.Dmatrix[d].middleRows(localSpace.NumBoundaryBasisFunctions,
...
//****************************************************************************
void VEM_DF_PCC_3D_Velocity_LocalSpace::ComputeStabilizationMatrix(const double& polyhedronDiameter,
                                                                   VEM_DF_PCC_3D_Velocity_LocalSpace_Data& localSpace) const
{
    Eigen::MatrixXd staBmatrix = localSpace.Dmatrix[0] * localSpace.PiNabla[0]
                                 + localSpace.Dmatrix[1] * localSpace.PiNabla[1]
                                 + localSpace.Dmatrix[2] * localSpace.PiNabla[2];

    staBmatrix.diagonal().array() -= 1;

    // staBmatrix = (\Pi^{0,dofs}_order - I)^T * (\Pi^{0,dofs}_order - I).
    localSpace.StabMatrix = polyhedronDiameter * staBmatrix.transpose() * staBmatrix;
}
//****************************************************************************
void VEM_DF_PCC_3D_Velocity_LocalSpace::ComputeCMatrixkm2(const double& polyhedronMeasure,
                                                          const double& polyhedronDiameter,
...
//****************************************************************************
void VEM_DF_PCC_3D_Velocity_LocalSpace::ComputeL2Projectors(const Eigen::VectorXd& internalQuadratureWeights,
                                                            VEM_DF_PCC_3D_Velocity_LocalSpace_Data& localSpace) const
{
    // DOF big o plus
    ...
    MatrixXd tmpE = MatrixXd::Zero(3 * (localSpace.Nkm1 - localSpace.Nkm3) + localSpace.Nkm4 - localSpace.Nkm2, localSpace.NumBasisFunctions);
    tmpE << EnhancedMassMatrix.middleRows(localSpace.Nkm3 - localSpace.Nkm4, localSpace.Nkm1 - localSpace.Nkm3 + localSpace.Nkm4 - localSpace.Nkm2),
        EnhancedMassMatrix.middleRows(localSpace.Nkm1 - localSpace.Nkm2 + localSpace.Nkm3, localSpace.Nkm1 - localSpace.Nkm3),
        EnhancedMassMatrix.middleRows(2 * localSpace.Nkm1 - localSpace.Nkm2 + localSpace.Nkm3, localSpace.Nkm1 - localSpace.Nkm3);

    for(unsigned int d = 0; d < localSpace.Dimension; d++)
    {

        MatrixXd tmpVD = MatrixXd::Zero(localSpace.Nk - localSpace.Nkm2, 3 * (localSpace.Nkm1 - localSpace.Nkm3) + localSpace.Nkm4 - localSpace.Nkm2);
        tmpVD << localSpace.VectorDecompositionMatrices[d][1].block(localSpace.Nkm2, localSpace.Nkm3 - localSpace.Nkm4,
                                                                    localSpace.Nk - localSpace.Nkm2, localSpace.Nkm1 - localSpace.Nkm3 + localSpace.Nkm4 - localSpace.Nkm2),
            localSpace.VectorDecompositionMatrices[d][1].block(localSpace.Nkm2, localSpace.Nkm1 - localSpace.Nkm2 + localSpace.Nkm3,
                                                               localSpace.Nk - localSpace.Nkm2, localSpace.Nkm1 - localSpace.Nkm3),
            localSpace.VectorDecompositionMatrices[d][1].block(localSpace.Nkm2, 2 * localSpace.Nkm1 - localSpace.Nkm2 + localSpace.Nkm3,
                                                               localSpace.Nk - localSpace.Nkm2, localSpace.Nkm1 - localSpace.Nkm3);
    ...
    const Eigen::LLT<Eigen::MatrixXd> Hmatrix_km2_llt = localSpace.Hmatrix.topLeftCorner(localSpace.Nkm2, localSpace.Nkm2).llt();
...
//****************************************************************************
void VEM_DF_PCC_3D_Velocity_LocalSpace::ComputePiNabla(const VEM_DF_PCC_3D_Velocity_ReferenceElement_Data &reference_element_data,
                                                       const double& polyhedronMeasure,
                                                       const double& polyhedronDiameter,
                                                       const Eigen::VectorXd& internalQuadratureWeights,
...
{ 
    ...
    localSpace.Gmatrix = localSpace.VanderInternalDerivatives[0].transpose() *
                         ...
                         localSpace.VanderInternalDerivatives[2].transpose() *
                             internalQuadratureWeights.asDiagonal() *
                             localSpace.VanderInternalDerivatives[2];

    localSpace.Gmatrix.row(0) = ( 1.0 / polyhedronMeasure) * localSpace.VanderInternal.transpose() *
                                internalQuadratureWeights;

    // B_{ij} = \int_E \nabla m_i \nabla \phi_j
    const MatrixXd BmatrixX = localSpace.VanderBoundaryDerivatives[0].transpose() *
                              ...
                              localSpace.VanderBoundaryDerivatives[2].transpose() *
                                  boundaryQuadratureWeightsTimesNormal[2].asDiagonal();
...
//****************************************************************************
void VEM_DF_PCC_3D_Velocity_LocalSpace::ComputeL2ProjectorsOfDerivatives(const VEM_DF_PCC_3D_Velocity_ReferenceElement_Data &reference_element_data,
                                                                         const double &polyhedronDiameter,
                                                                         const std::vector<Eigen::VectorXd>& boundaryQuadratureWeightsTimesNormal,
                                                                         VEM_DF_PCC_3D_Velocity_LocalSpace_Data& localSpace) const
{
    ...
    for(unsigned int d1 = 0; d1 < localSpace.Dimension; d1++)
    {
        for(unsigned int d2 = 0; d2 < localSpace.Dimension; d2++)
        {
            localSpace.Ematrix[localSpace.Dimension * d1 + d2] =
                -invDiameter * monomials.DerivativeMatrix(reference_element_data.Monomials, d2).topLeftCorner(localSpace.Nkm1,
                                                                                                              localSpace.Nkm2) * localSpace.Cmatrixkm2[d1];
            localSpace.Ematrix[localSpace.Dimension * d1 + d2].leftCols(localSpace.NumBoundaryBasisFunctions) +=
                localSpace.VanderBoundary.leftCols(localSpace.Nkm1).transpose() * boundaryQuadratureWeightsTimesNormal[d2].asDiagonal()
                * localSpace.VanderFaceProjectionsKm1[d1];
        }
    }
