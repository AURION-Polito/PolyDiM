...
template <unsigned short dimension> struct VEM_MCC_Utilities final
{
    ...
    {
        Eigen::MatrixXd polynomialBasisDofs = Eigen::MatrixXd::Zero(NumBasisFunctions, dimension * Nk);
    ...
    {
        Eigen::MatrixXd stabMatrixPi0k = DMatrix * projector;
                              ...
                              std::vector<Eigen::MatrixXd> &Cmatrixkp1) const
    {
        ...
        {
            Cmatrixkp1[e] = Eigen::MatrixXd::Zero(nkp1, order_1D);

            const Eigen::Vector3d &edgeStart = edgeDirections[e] ? polygonVertices.col(e)
                                                                 : polygonVertices.col((e + 1) % numVertices);
            const Eigen::Vector3d &edgeTangent = edgeTangents.col(e);
            ...
            {
                Eigen::VectorXd vettX = Eigen::VectorXd::Ones(polynomialDegree + 2); // ((xa - xc)/diam)^{alphax}
                Eigen::VectorXd vettY = Eigen::VectorXd::Ones(polynomialDegree + 2); // ((ya - yc)/diam)^{alphay}
                Eigen::VectorXd vettDiffX = Eigen::VectorXd::Ones(polynomialDegree + 2);
                Eigen::VectorXd vettDiffY = Eigen::VectorXd::Ones(polynomialDegree + 2);
            ...
            {
                Eigen::VectorXd vettX = Eigen::VectorXd::Ones(polynomialDegree + 2); // ((xa - xc)/diam)^{alphax}
                Eigen::VectorXd vettY = Eigen::VectorXd::Ones(polynomialDegree + 2); // ((yB - yA)/diam)^{alphay}
                Eigen::VectorXd vettDiffX = Eigen::VectorXd::Ones(polynomialDegree + 2);
            ...
            {
                Eigen::VectorXd vettX = Eigen::VectorXd::Ones(polynomialDegree + 2); // ((xB - xA)/diam)^{alphax}
                Eigen::VectorXd vettY = Eigen::VectorXd::Ones(polynomialDegree + 2); // ((ya - yc)/diam)^{alphay}
                Eigen::VectorXd vettDiffY = Eigen::VectorXd::Ones(polynomialDegree + 2);
