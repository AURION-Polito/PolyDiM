...
{
//****************************************************************************
VEM_MCC_VelocityLocalSpace_Data VEM_MCC_3D_VelocityLocalSpace::CreateLocalSpace(const VEM_MCC_3D_Velocity_ReferenceElement_Data& reference_element_data,
                                                                                const VEM_MCC_3D_Polyhedron_Geometry& polyhedron) const
{
    ...
    localSpace.InternalQuadrature = quadrature.PolyhedronInternalQuadrature(reference_element_data.Quadrature,
                                                                            polyhedron.GeometryUtility,
                                                                            polyhedron.TetrahedronVertices);

    Quadrature::VEM_Quadrature_3D::Faces_QuadratureData_MCC boundaryQuadrature = quadrature.PolyhedronFacesQuadrature(reference_element_data.Quadrature,
                                                                                                                      polyhedron.GeometryUtility,
    ...
    InitializeProjectorsComputation(reference_element_data,
                                    ...
                                    boundaryQuadrature.BoundaryQuadrature.Points,
                                    localSpace);
    ...
    ComputeValuesOnBoundary(reference_element_data,
                            ...
                            boundaryQuadrature.FacesQuadrature,
                            boundaryQuadrature.BoundaryQuadrature.Weights,
                            W2,
                            B2Nabla,
                            localSpace);

    ComputeDivergenceCoefficients(polyhedron.Measure,
                                  W2,
                                  localSpace);
    ...
    ComputeStabilizationMatrix(polyhedron.Measure,
                               localSpace);

    return localSpace;
}
//****************************************************************************
void VEM_MCC_3D_VelocityLocalSpace::InitializeProjectorsComputation(const VEM_MCC_3D_Velocity_ReferenceElement_Data& reference_element_data,
                                                                    ...
                                                                    VEM_MCC_VelocityLocalSpace_Data& localSpace) const
{
    ...
    localSpace.NkNabla = localSpace.Nk + (reference_element_data.Order + 2)
                                             * (reference_element_data.Order + 3) / 2 - 1;
    ...
    localSpace.VanderInternalKp1 = monomials3D.Vander(reference_element_data.MonomialsKp1,
    ...
    localSpace.VanderBoundaryKp1 = monomials3D.Vander(reference_element_data.MonomialsKp1,
    ...
    const MatrixXd dx = (1.0 / polyhedronDiameter) * monomials3D.D_x(reference_element_data.MonomialsKp1).bottomLeftCorner(localSpace.NkNabla,
                                                                                                                           localSpace.Nk);
    const MatrixXd dy = (1.0 / polyhedronDiameter) * monomials3D.D_y(reference_element_data.MonomialsKp1).bottomLeftCorner(localSpace.NkNabla,
                                                                                                                           localSpace.Nk);
    const MatrixXd dz = (1.0 / polyhedronDiameter) * monomials3D.D_z(reference_element_data.MonomialsKp1).bottomLeftCorner(localSpace.NkNabla,
                                                                                                                           localSpace.Nk);
    ...
    VanderInternalEnlarged << localSpace.VanderInternal.transpose(), zeros, zeros,
        zeros, localSpace.VanderInternal.transpose(), zeros,
        zeros, zeros, localSpace.VanderInternal.transpose();
...
//****************************************************************************
void VEM_MCC_3D_VelocityLocalSpace::ComputeDivergenceCoefficients(const double& polyhedronMeasure,
...
//****************************************************************************
void VEM_MCC_3D_VelocityLocalSpace::ComputeL2Projectors(const double& polyhedronMeasure,
                                                        const Eigen::VectorXd& internalQuadratureWeights,
                                                        ...
                                                        VEM_MCC_VelocityLocalSpace_Data& localSpace) const
{
    ...
    BBigOPlus.rightCols(localSpace.NumBigOPlusInternalBasisFunctions)
        = polyhedronMeasure * Eigen::MatrixXd::Identity(localSpace.NumBigOPlusInternalBasisFunctions,localSpace.NumBigOPlusInternalBasisFunctions);
...
//****************************************************************************
void VEM_MCC_3D_VelocityLocalSpace::ComputeValuesOnBoundary(const VEM_MCC_3D_Velocity_ReferenceElement_Data& reference_element_data,
                                                            const unsigned int &numFaces,
                                                            ...
                                                            VEM_MCC_VelocityLocalSpace_Data &localSpace) const
{
    ...
    MatrixXd VanderBoundaryEnlarged(localSpace.Dimension * localSpace.Nk,
                                    localSpace.Dimension * numBoundaryQuadraturePoints);

    const MatrixXd zeros = MatrixXd::Zero(localSpace.VanderBoundary.cols(), localSpace.VanderBoundary.rows());

    VanderBoundaryEnlarged << localSpace.VanderBoundary.transpose(), zeros, zeros,
        zeros, localSpace.VanderBoundary.transpose(), zeros,
        zeros, zeros, localSpace.VanderBoundary.transpose();
    ...
    for(unsigned int d = 0; d < localSpace.Dimension; d++)
        GkVanderBoundary_comp[d] = GkVanderBoundary.block(0, d * numBoundaryQuadraturePoints,
                                                          localSpace.Dimension * localSpace.Nk, numBoundaryQuadraturePoints);

    localSpace.GkVanderBoundaryTimesNormal = MatrixXd::Zero(localSpace.NumBoundaryBasisFunctions,
                                                            localSpace.Dimension * localSpace.Nk);

    W2 = MatrixXd::Zero(localSpace.Nk,
                        localSpace.NumBasisFunctions);
    ...
    for(unsigned int f = 0; f < numFaces; f++)
    {
        ...
        localSpace.VanderBasisFunctionValuesOnFace[f] = facesAreas[f] * localSpace.FacesVanderInternal[f] * faceHmatrix.llt().solve(id);

        W2.block(0, f * nk2D,
                 localSpace.Nk, nk2D) = globalDirection
              * localSpace.VanderBoundary.block(offsetQuadraturePoints, 0,
                                                numFaceQuadraturePoints, localSpace.Nk).transpose()
              * facesQuadrature[f].Weights.asDiagonal()
              * localSpace.VanderBasisFunctionValuesOnFace[f];

        B2Nabla.block(0, f * nk2D,
                      localSpace.NkNabla, nk2D) = globalDirection
              * localSpace.VanderBoundaryKp1.rightCols(localSpace.NkNabla).transpose().block(0,offsetQuadraturePoints,
                                                                                             localSpace.NkNabla, numFaceQuadraturePoints)
              * facesQuadrature[f].Weights.asDiagonal()
              * localSpace.VanderBasisFunctionValuesOnFace[f];

        localSpace.GkVanderBoundaryTimesNormal.block(f * nk2D, 0,
                                                     nk2D, 3 * localSpace.Nk) = (1.0/ facesAreas[f]) * localDirection * globalDirection
              * localSpace.FacesVanderInternal[f].transpose()
              * boundaryQuadratureWeights.segment(offsetQuadraturePoints, numFaceQuadraturePoints).asDiagonal()
              * (facesNormals[f](0) * GkVanderBoundary_comp[0].block(0, offsetQuadraturePoints,
                                                                     3 * localSpace.Nk, numFaceQuadraturePoints) +
                 facesNormals[f](1) * GkVanderBoundary_comp[1].block(0, offsetQuadraturePoints,
                                                                     3 * localSpace.Nk, numFaceQuadraturePoints) +
                 facesNormals[f](2) * GkVanderBoundary_comp[2].block(0, offsetQuadraturePoints,
                                                                     3 * localSpace.Nk, numFaceQuadraturePoints)).transpose();
