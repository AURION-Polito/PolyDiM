...
{
//****************************************************************************
VEM_MCC_VelocityLocalSpace_Data VEM_MCC_2D_Ortho_VelocityLocalSpace::CreateLocalSpace(const VEM_MCC_2D_ReferenceElement_Data& reference_element_data,
...
//****************************************************************************
void VEM_MCC_2D_Ortho_VelocityLocalSpace::InitializeProjectorsComputation(const VEM_MCC_2D_ReferenceElement_Data& reference_element_data,
                                                                          ...
                                                                          VEM_MCC_VelocityLocalSpace_Data& localSpace) const
{
    ...
    ChangeOfBasis(VanderInternalMonomials,
                  internalQuadratureWeights,
                  localSpace);

    // Compute Vandermonde matrices.
    localSpace.VanderInternalKp1 = VanderInternalMonomials
                                   * localSpace.QmatrixKp1.transpose();

    localSpace.VanderInternal = localSpace.VanderInternalKp1.leftCols(localSpace.Nk);

    const MatrixXd VanderBoundaryMonomials = monomials.Vander(reference_element_data.MonomialsKp1,
                                                              boundaryQuadraturePoints,
                                                              polygonCentroid,
                                                              polygonDiameter);


    localSpace.VanderBoundaryKp1 = VanderBoundaryMonomials
                                   * localSpace.QmatrixKp1.transpose();
    ...
    dx = (localSpace.QmatrixKp1 * dx *
          localSpace.QmatrixInvKp1.topLeftCorner(localSpace.Nk,localSpace.Nk)).bottomLeftCorner(localSpace.NkNabla,
                               localSpace.Nk);
    dy = (localSpace.QmatrixKp1 * dy *
          localSpace.QmatrixInvKp1.topLeftCorner(localSpace.Nk,localSpace.Nk)).bottomLeftCorner(localSpace.NkNabla,
                               localSpace.Nk);
    ...
    MatrixXd producttt = Q2.transpose() * Q2;
...
//****************************************************************************
void VEM_MCC_2D_Ortho_VelocityLocalSpace::ChangeOfBasis(const Eigen::MatrixXd &VanderInternalMonomialsKp1,
                                                        const Eigen::VectorXd& internalQuadratureWeights,
                                                        VEM_MCC_VelocityLocalSpace_Data& localSpace) const
{
...
//****************************************************************************
void VEM_MCC_2D_Ortho_VelocityLocalSpace::ComputeL2Projectors(const double& polygonMeasure,
                                                              const Eigen::VectorXd& internalQuadratureWeights,
...
//****************************************************************************
void VEM_MCC_2D_Ortho_VelocityLocalSpace::ComputeDivergenceCoefficients(const double& polytopeMeasure,
                                                                        ...
                                                                        VEM_MCC_VelocityLocalSpace_Data& localSpace) const
{
    ...
    if(localSpace.Order > 0 )
    {
        W1.block(1,
                 ...
                 localSpace.NumNablaInternalBasisFunctions)
            = - polytopeMeasure *
              localSpace.QmatrixTkNablaInv.topLeftCorner(localSpace.NumNablaInternalBasisFunctions,
                                                         localSpace.NumNablaInternalBasisFunctions);
    }
...
//****************************************************************************
void VEM_MCC_2D_Ortho_VelocityLocalSpace::ComputeValuesOnBoundary(const Eigen::MatrixXd& polytopeVertices,
                                                                  ...
                                                                  VEM_MCC_VelocityLocalSpace_Data& localSpace) const
{
    ...
    MatrixXd VanderBoundary2k(localSpace.Dimension * localSpace.Nk, localSpace.Dimension * boundaryQuadratureWeights.size());
