...
public:
    VEM_MCC_2D_Velocity_ReferenceElement_Data Create(const unsigned int order) const
    {
        ...
        {
            Gedim::Quadrature::QuadratureData referenceQuadrature1D = Gedim::Quadrature::Quadrature_Gauss1D::FillPointsAndWeights(2 * order + 2);

            const Eigen::VectorXd sqrtInternalQuadratureWeights1D = referenceQuadrature1D.Weights.array().sqrt();
            const Eigen::MatrixXd VanderBoundary1Dkp1 = monomials_1D.Vander(result.Monomials_1D,
                                                                            referenceQuadrature1D.Points,
                                                                            Eigen::Vector3d::Constant(0.0),
                                                                            1.0);

            Eigen::MatrixXd Q1_1D;
            Eigen::MatrixXd R1_1D;
            LAPACK_utilities::MGS(VanderBoundary1Dkp1,
                                  Q1_1D,
                                  R1_1D);
