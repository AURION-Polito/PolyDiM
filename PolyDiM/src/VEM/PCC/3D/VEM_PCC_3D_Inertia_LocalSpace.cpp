    ...
    const VEM_PCC_3D_Polyhedron_Geometry &polyhedron) const
{
    ...
    InertiaMapping(geometryUtilities,
                   polyhedron,
    ...
    for (unsigned int f = 0; f < numFaces; f++)
    {
        ...
        facesMappedFacesQuadratureWeights[f] = localSpace.facesLocalSpace[f].InternalQuadrature.Weights / localSpace.facesLocalSpace[f].inertia_data.absDetFmatrix;
        facesMeasure[f] = localSpace.facesLocalSpace[f].inertia_data.Measure;
    }

    Gedim::Quadrature::QuadratureData MappedInternalQuadrature = quadrature3D.PolyhedronInternalQuadrature(reference_element_data_3D.Quadrature,
                                                                                                           geometryUtilities,
    ...
    Quadrature::VEM_Quadrature_3D::Faces_QuadratureData_PCC MappedBoundaryQuadrature = quadrature3D.PolyhedronFacesQuadrature(geometryUtilities,
    ...
    MappedBoundaryQuadrature.Quadrature.Points = localSpace.inertia_data.FmatrixInv * (MappedBoundaryQuadrature.Quadrature.Points.colwise()
                                                                                       - localSpace.inertia_data.translation);
    ...
    const MatrixXd edgeInternalQuadraturePoints = (localSpace.inertia_data.Fmatrix * mappedEdgeInternalQuadraturePoints).colwise() +
                                                  localSpace.inertia_data.translation;
                                                   ...
                                                   VEM_PCC_3D_Inertia_Data &inertia_data) const
{
    ...
    inertia_data.absDetFmatrix *= polyhedron.Diameter *
                                  polyhedron.Diameter *
                                  polyhedron.Diameter;
    inertia_data.translation += Vector3d::Zero();

    const MatrixXd polyhedronVerticesFirstRescaling =  inertia_data.FmatrixInv * (polyhedron.Vertices.colwise() - inertia_data.translation);
    ...
    {
        polyhedronTetrahedronFirstRescaling[n] = inertia_data.FmatrixInv * (polyhedron.TetrahedronVertices[n].colwise() - inertia_data.translation);
    }

    const Matrix3d HmatrixFirstRescaling = geometryUtilities.PolyhedronMass(polyhedronCentroidsFirstRescaling,
                                                                            polyhedronTetrahedronFirstRescaling);
    ...
    const Matrix3d Bmatrix = sqrtLambdaInvFirstRescaling.asDiagonal() * QmatrixFirstRescaling.transpose() * sqrtLambdaFirstRescaling.maxCoeff();
    const Matrix3d BmatrixInv = QmatrixFirstRescaling * sqrtLambdaFirstRescaling.asDiagonal() / sqrtLambdaFirstRescaling.maxCoeff();
    ...
    const MatrixXd polyhedronVerticesInertiaMapping =  inertia_data.FmatrixInv * (polyhedron.Vertices.colwise() - inertia_data.translation);
    ...
    inertia_data.absDetFmatrix *= polyhedronDiameterInertiaMapping *
                                  polyhedronDiameterInertiaMapping *
                                  polyhedronDiameterInertiaMapping;
    inertia_data.translation += Vector3d::Zero();
    ...
    const std::vector<Eigen::MatrixXd>& faces3DVerticesSecondRescaling =
        geometryUtilities.PolyhedronFaceVertices(inertia_data.Vertices,
                                                 polyhedron.Faces);
    ...
    ComputeGeometryProperties(geometryUtilities,
                              ...
                              faces3DVerticesSecondRescaling,
                              inertia_data);
}
// ***************************************************************************
                                                              ...
                                                              VEM_PCC_3D_Inertia_Data &inertia_data) const
{
    ...
    inertia_data.EdgesTangent = geometryUtilities.PolyhedronEdgeTangents(inertia_data.Vertices,
                                                                         polyhedronEdges);
    ...
    inertia_data.FacesNormalDirection = geometryUtilities.PolyhedronFaceNormalDirections(mappedFaces3DVertices,
                                                                                         geometryUtilities.PolyhedronBarycenter(inertia_data.Vertices),
                                                                                         inertia_data.FacesNormal);

    inertia_data.FacesRotationMatrix = geometryUtilities.PolyhedronFaceRotationMatrices(mappedFaces3DVertices,
                                                                                        inertia_data.FacesNormal,
    ...
    const vector<vector<unsigned int>> polyhedronFaceTriangulations = geometryUtilities.PolyhedronFaceTriangulationsByFirstVertex(polyhedronFaces,
                                                                                                                                  mappedFaces3DVertices);
    ...
    std::vector<std::vector<Matrix3d>> faces2DTriangulations = geometryUtilities.PolyhedronFaceExtractTriangulationPoints(faces2DVertices,
    ...
    const vector<unsigned int> polyhedronTetrahedrons = geometryUtilities.PolyhedronTetrahedronsByFaceTriangulations(inertia_data.Vertices,
                                                                                                                     polyhedronFaces,
                                                                                                                     polyhedronFaceTriangulations,
                                                                                                                     inertia_data.Centroid);

    inertia_data.TetrahedronVertices = geometryUtilities.ExtractTetrahedronPoints(inertia_data.Vertices,
                                                                                  inertia_data.Centroid,
                                                                    ...
                                                                    VEM_PCC_3D_LocalSpace_Data &localSpace) const
{
    ...
    localSpace.NumProjectorBasisFunctions = (localSpace.Order + 1) *
                                            (localSpace.Order + 2) *
                                            (localSpace.Order + 3) / 6;
