    ...
    const FEM_PCC_3D_Polyhedron_Geometry &polyhedron) const
{
    ...
    for (unsigned int f = 0; f < 4; ++f)
    {
        ...
        if (reference_element_data.NumDofs2D > 0)
        {

            const unsigned int local_space_2D_face_dof_index =
                3 * reference_element_data.NumDofs0D + 3 * reference_element_data.NumDofs1D;

            const auto &local_space_2D = localSpace.Boundary_LocalSpace_Data[f];
            const Eigen::MatrixXd face_internal_dofs_2D =
                local_space_2D.Dofs.block(0, local_space_2D_face_dof_index, 3, reference_element_data.NumDofs2D);
            ...
            const auto face_internal_dofs_3D_from_reference_2D =
                geometry_utilities.RotatePointsFrom2DTo3D(face_internal_dofs_2D, face_rotation_matrix, face_translation);

            const Eigen::MatrixXd reference_face_dofs =
                reference_element_data.DofPositions.block(0, face_dof_counter, 3, reference_element_data.NumDofs2D);
