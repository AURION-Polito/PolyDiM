  ...
  namespace FEM
  {
    namespace PCC
    {
          ...
          const FEM_Triangle_PCC_2D_Polygon_Geometry &polygon) const
      {
        ...
        localSpace.BoundaryQuadrature = BoundaryQuadrature(reference_element_data.BoundaryReferenceElement_Data.ReferenceSegmentQuadrature,
                                                           polygon);
      ...
      // ***************************************************************************
      std::vector<Gedim::Quadrature::QuadratureData> FEM_Triangle_PCC_2D_LocalSpace::BoundaryQuadrature(const Gedim::Quadrature::QuadratureData& reference_quadrature,
                                                                                                        const FEM_Triangle_PCC_2D_Polygon_Geometry& polygon) const
      {
        ...
        for (unsigned int e = 0; e < num_edges; ++e)
        {
          ...
          const Eigen::Vector3d edge_origin = edge_direction ?
                                                polygon.Vertices.col(e) :
                                                polygon.Vertices.col((e + 1) % num_edges);

          const Eigen::Vector3d edge_tangent = edge_direction ?
          ...
          for (unsigned int q = 0; q < num_quadrature_points; q++)
            edge_quadrature.Points.col(q) = edge_origin +
                                            reference_quadrature.Points(0, q) *
                                            edge_tangent;

          edge_quadrature.Weights = reference_quadrature.Weights *
                                                        std::abs(edge_length);
        }
