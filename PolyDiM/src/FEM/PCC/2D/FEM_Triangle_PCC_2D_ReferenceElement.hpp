        ...
        public:
          FEM_Triangle_PCC_2D_ReferenceElement_Data Create(const unsigned int order) const
          {
            ...
            if (order == 0)
            {
              ...
              result.DofTypes.setZero(3,
                                      result.NumBasisFunctions);
              result.DofPositions.setZero(3,
                                          result.NumBasisFunctions);
            ...
            Eigen::MatrixXd localDofPositions = Eigen::MatrixXd::Zero(3,
                                                                      result.NumBasisFunctions);
            Eigen::MatrixXi localDofTypes = Eigen::MatrixXi::Zero(3,
                                                                  result.NumBasisFunctions);
            ...
            result.ReferenceBasisFunctionValues = EvaluateBasisFunctions(result.ReferenceTriangleQuadrature.Points,
                                                                         result);
            result.ReferenceBasisFunctionDerivativeValues = EvaluateBasisFunctionDerivatives(result.ReferenceTriangleQuadrature.Points,
                                                                                             result);
            ...
            switch (reference_element_data.Order)
            {
              ...
              default:
              {
                ...
                Eigen::MatrixXd values = Eigen::MatrixXd::Ones(points.cols(),
                                                               reference_element_data.NumBasisFunctions);
            ...
            switch (reference_element_data.Order)
            {
              case 0:
                return std::vector<Eigen::MatrixXd>(reference_element_data.Dimension,
                                                    Eigen::MatrixXd::Zero(points.cols(),
                                                                          reference_element_data.NumBasisFunctions));
              default:
              {
                const double h = 1.0 / reference_element_data.Order;
